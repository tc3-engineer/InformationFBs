...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187757323.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_2OCTET_SIGN_SEND.TcPOU`](../examples/P_Demo_EIB_2OCTET_SIGN_SEND.TcPOU) |

---

## 1. 功能简述

**发送 DPT 8.xxx (2-byte signed int) telegram**：把 IEC `INT` 值经库编码成 EIB 标准格式，发送到指定 `Group_Address`。**只在 `iData` 值变化时下发**——周期重发要用 `EIB_2OCTET_SIGN_SEND_EX`。
...
> 配套可导入文件：[`examples/P_Demo_EIB_2OCTET_SIGN_SEND.TcPOU`](../examples/P_Demo_EIB_2OCTET_SIGN_SEND.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）

> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_EIB_2OCTET_SIGN_SEND
VAR
    fb       : EIB_2OCTET_SIGN_SEND;
    stEibRec : EIB_REC;
...
- **场景**：PLC 温度偏置（±32768 范围）发送到 BMS
- **价值**：DPT DPT 8.xxx 标准发送方式
- **替代方案对比**：
  - `EIB_2OCTET_SIGN_SEND_EX`：周期发 + read 响应
  - `EIB_ALL_DATA_TYPES_SEND`：raw byte 模式
  - 本 FB：简单变化发送的标准选择

## 8. 参考资料

- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.3
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187757323.html
- **相关**：`EIB_2OCTET_SIGN_SEND_EX`（同库 §4.2.5.4）、对应接收 FB
