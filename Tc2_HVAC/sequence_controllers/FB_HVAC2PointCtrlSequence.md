...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685064331.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVAC2PointCtrlSequence.TcPOU`](../examples/P_Demo_FB_HVAC2PointCtrlSequence.TcPOU) |

---

## 1. 功能简述
序列控制器 FB（多段加热 / 冷却 / 加湿 / 能量回收等串联）：`FB_HVAC2PointCtrlSequence` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 10 个 VAR_INPUT 引脚与 9 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `e2PointCtrlMode` | `E_HVAC2PointCtrlMode` | - | 枚举 / 结构（参见 `E_HVAC2PointCtrlMode`）。 |
...
| `bActiveCtrl` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `b_rW_Max` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `b_rW_Min` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `rE` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `e2PointCtrlState` | `E_HVAC2PointCtrlMode` | - | 枚举 / 结构（参见 `E_HVAC2PointCtrlMode`）。 |
| `st2PointCtrlSequence` | `ST_HVAC2PointCtrlSequence` | - | 枚举 / 结构（参见 `ST_HVAC2PointCtrlSequence`）。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVAC2PointCtrlSequence.TcPOU`](../examples/P_Demo_FB_HVAC2PointCtrlSequence.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.4.7.1
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685064331.html
