...
| Source | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_standard/right.html |
...
VAR
    sResult : STRING(255);   // RIGHT 的返回值（监视）
    bRun    : BOOL;          // TRUE 触发一次调用
END_VAR

IF bRun THEN
    sResult := RIGHT('SUSI',3);
    bRun := FALSE;
END_IF;
