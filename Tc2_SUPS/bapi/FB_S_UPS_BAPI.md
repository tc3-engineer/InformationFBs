...
`FB_S_UPS_BAPI` 用于 **BAPI** 平台带 1-second UPS 设备从 PLC 控制 UPS。断电时根据 `eUpsMode` 决定保存持久化数据或/和执行 quick shutdown。**默认输入值不应改动**。
