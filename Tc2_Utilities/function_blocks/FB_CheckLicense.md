...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/34977419.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_CheckLicense.TcPOU`](../examples/P_Demo_FB_CheckLicense.TcPOU) |

---

## 1. 功能简述

FB_CheckLicense 通过 TwinCAT License Manager 查询某个 License ID 是否在本机有效。返回 BOOL 表示是否激活 + 详细状态（试用 / 永久 / 即将过期 / 已过期）。
...
| `stLicenseId` | `GUID` | - | 参数 `stLicenseId`（类型 `GUID`）。⚠️ PDF 未详述含义，请按 §3 行为说明使用。 |
...
| `stCheckLicense` | `ST_CheckLicense` | 参数 `stCheckLicense`（类型 `ST_CheckLicense`）。⚠️ PDF 未详述含义，请按 §3 行为说明使用。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_CheckLicense.TcPOU`](../examples/P_Demo_FB_CheckLicense.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）。
>
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf) §3.11
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/34977419.html
- **相关 FB**：`FB_CheckOemLicense`, `FB_GetLicenses`
