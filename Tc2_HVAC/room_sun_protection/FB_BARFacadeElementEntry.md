...
| Example | [`examples/P_Demo_FB_BARFacadeElementEntry.TcPOU`](../examples/P_Demo_FB_BARFacadeElementEntry.TcPOU) |

---

## 1. 功能简述
房间遮阳 FB（百叶 / 卷帘 / 阴影矫正 / 立面元素等）：`FB_BARFacadeElementEntry` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 10 个 VAR_INPUT 引脚与 8 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `iColumn` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `iRow` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `bWrite` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bRead` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `usiGroup` | `USINT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `lrCorner1X` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `lrCorner1Y` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `lrWindowWidth` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `lrWindowHeight` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |

#### VAR_OUTPUT 引脚描述

| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `lrCorner2X` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `lrCorner2Y` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `lrCorner3X` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `lrCorner3Y` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `lrCorner4X` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `lrCorner4Y` | `LREAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_BARFacadeElementEntry.TcPOU`](../examples/P_Demo_FB_BARFacadeElementEntry.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.5.4.7
