...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc3_eventlogger/5051447947.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_OnAlarmCleared.TcPOU`](../examples/P_Demo_OnAlarmCleared.TcPOU) |
...
本方法是事件驱动回调（callback），由 EventLogger 在 alarm 状态从 Raised 转 Cleared 时 调用。调用上下文是 PLC 任务（调用 listener.Execute() 的任务），因此回调里的代码占用 PLC 周期时间。
...
> 配套可导入文件：[`examples/P_Demo_OnAlarmCleared.TcPOU`](../examples/P_Demo_OnAlarmCleared.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
计算故障持续时长指标：在 OnAlarmRaised 记开始时间，OnAlarmCleared 算差值入数据库
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc3_EventLogger_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_EventLogger_EN.pdf) §3.5.2
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc3_eventlogger/5051447947.html
