...
| Status | `verified` |
| Example | [`examples/P_Demo_FB_XmlDomParser_FirstNodeByPath.TcPOU`](../examples/P_Demo_FB_XmlDomParser_FirstNodeByPath.TcPOU) |

---

## 1. 功能简述

`FB_XmlDomParser.FirstNodeByPath()` 是 XML DOM 文档 提供的一个工具方法，在解析/构造 JSON 或 XML 文档的特定步骤中使用。具体参数语义见下文 §2 接口定义表格与 §3 行为说明；参数语义按命名约定推导自该方法在 `FB_XmlDomParser` 内的位置。
...
| `path` | `STRING` | JSON 路径字符串（如 `'a.b.c'`）。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_XmlDomParser_FirstNodeByPath.TcPOU`](../examples/P_Demo_FB_XmlDomParser_FirstNodeByPath.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **PDF**：[`TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf) §4.7.47
