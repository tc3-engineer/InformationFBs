...
3. 每篇配套 `examples/P_Demo_<...>.xml`，`lint_plcopen.py` 退出码 0。
4. OO method 与父 FB 同章节文本内被 `parse_toc` 拆分；`verify_doc` 通过 `is_parent` + `parent_dir` 双路径定位。

## 已知细节

- **同名 OO 方法**：`Create` / `CreateEx` / `Clear` / `SetJsonAttribute` 同名同形参出现在多个 parent（FB_TcAlarm、FB_TcMessage、FB_TcSourceInfo）。文档保留 method 名做文件名，例程文件统一用 `P_Demo_<Parent>_<Method>.xml` 避免覆盖；文档头部 Example 字段已对应。
