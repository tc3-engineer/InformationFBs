...
| Type | `FUNCTION_BLOCK` |
| Category | `Client · Remote Analog Value` |
...
| Example | [`examples/P_Demo_FB_BACnetRM_AV.TcPOU`](../examples/P_Demo_FB_BACnetRM_AV.TcPOU) |

---

## 1. 功能简述

代表远端 BACnet 设备中一个「Analog Value」对象的引用。Client 端通过本 FB 周期读 / 写 peer 设备的对应对象,把远端 Present_Value 拉到 PLC 内变量(读)或把 PLC 内变量推到远端(写)。命名规则 `FB_BACnetRM_<shortcut>`(PDF §5.3.1 / §7.3)。
...
> 配套可导入文件:[`examples/P_Demo_FB_BACnetRM_AV.TcPOU`](../examples/P_Demo_FB_BACnetRM_AV.TcPOU)

```iecst
PROGRAM P_Demo_FB_BACnetRM_AV
VAR
    fbClient : FB_BACnet_Client := (nDeviceInstance := 42);
    fbDevice : FB_BACnetRM_Device := (Client := fbClient);
    fbRemote : FB_BACnetRM_AV := (Client := fbClient, nObjectInstance := 1);
    val : REAL;
END_VAR
fbClient();
fbDevice();
...
- **场景**:把远端 BACnet 设备(如西门子 RDG 房间控制器)的 analog value 拉到本机 PLC 用 — 例如把对端房间温度引到本机 PID 做综合控制。
...
- **相关 FB**:`FB_BACnet_AV`(本机 AV)、`FB_BACnetRM_AI`(若对端是 AI 用这个);PDF §9.6 用 fbRM_AV.WritePropertyNull 复位远端槽位
