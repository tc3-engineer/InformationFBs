...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4699317003.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACCirculationPumpEx.TcPOU`](../examples/P_Demo_FB_HVACCirculationPumpEx.TcPOU) |

---

## 1. 功能简述
执行器控制 FB（阀门 / 风阀 / 水泵 / 电机）：`FB_HVACCirculationPumpEx` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 12 个 VAR_INPUT 引脚与 14 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `bExternal` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| `rValvePosition` | `REAL` | - | 当前阀门位置百分比 [%]，0..100。 |
| `bFeedbPump` | `BOOL` | - | 水泵实际运行反馈（如压差开关 / 电流互感器 DI）。 |
| `bPumpProtec` | `BOOL` | - | 水泵保护反馈（如电机过热触点）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `bPump` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `byState` | `BYTE` | - | 状态字（按位）：每一位对应一个联锁 / 状态信号，便于 HMI 统一回读多个布尔状态。 |
| `bReqExternal` | `BOOL` | - | 请求位（External），告诉上游 / 下游需要某个服务。 |
| `bReqOutsideTemp` | `BOOL` | - | 请求位（OutsideTemp），告诉上游 / 下游需要某个服务。 |
| `bReqValve` | `BOOL` | - | 请求位（Valve），告诉上游 / 下游需要某个服务。 |
| `bStateManSwitch` | `BOOL` | - | 状态镜像位（ManSwitch），用于把内部 / 输入状态回显给 HMI。 |
| `udiSecRT_StopDelay` | `UDINT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `udiSecRT_FeedbPumpDelay` | `UDINT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
...
| `bErrorPumpProtec` | `BOOL` | - | 错误指示位（PumpProtec）。`bReset` 上升沿清错。 |
...
| `bErrorPumpProtec` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACCirculationPumpEx.TcPOU`](../examples/P_Demo_FB_HVACCirculationPumpEx.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.2.4
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4699317003.html
