...
> ⚠️ **本功能块已废弃（[obsolete]，PDF §5.1.1）**。新工程请用 `ModbusRtuMasterV2_KL6x22B`（带功能码 23 `ReadWriteRegs` 与用户自定义报文 `UserReadWrite`）。本文档用于维护存量老程序。
...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6255_tc3_modbus_rtu/186542347.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_ModbusRtuMaster_KL6x22B.TcPOU`](../examples/P_Demo_ModbusRtuMaster_KL6x22B.TcPOU) |

---

## 1. 功能简述

通过串行总线端子 KL6031 或 KL6041 通讯的 Modbus RTU 主站功能块（Modbus master）的**旧版本**。它让 TwinCAT 控制器作为主站，主动向总线上的 Modbus 从站发起读写请求。功能与新版 `ModbusRtuMasterV2_KL6x22B` 类似，但**不含**功能码 23（`ReadWriteRegs`）和用户自定义报文（`UserReadWrite`），因此没有 `Aux*` 一组辅助参数。

本功能块**不以基本形式直接调用**，而是把每个 Modbus 功能码实现成一个**动作（Action）**。与端子的链接所需数据结构内置在 FB 中，分配方式与 TF6340 TwinCAT 3 Serial Communication 文档「Serial bus terminal」章节一致（经 PC 串口通讯请用旧版 `ModbusRtuMaster_PcCOM`；3 字节过程映像端子 KL6001/6011/6021 用旧版 `ModbusRtuMaster_KL6x5B`）。
...
- **已废弃，优先升级**：新工程用 `ModbusRtuMasterV2_KL6x22B`。本 FB 仅为兼容老程序保留。
- **`UnitID` 是 `UINT`**：可直接送集合地址 256/257/258。
- **端子组态**：KL6031/6041 串口参数用 KS2000 或 `Tc2_SerialCom` 的 `KL6configuration` 块设，波特率/校验位与从站一致。
- **`cbLength >= Quantity*2`**：字访问不足报 `MODBUSERROR_ILLEGAL_DATA_SIZE`。
- **缓冲区生命期**：`BUSY = TRUE` 期间不要改 `pMemoryAddr` 指向的变量。
- **端子选型**：22 字节过程映像端子（KL6031/6041）用本 FB；3 字节映像（KL6001/6011/6021）用旧版 `ModbusRtuMaster_KL6x5B`。（工程经验补充）

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_ModbusRtuMaster_KL6x22B.TcPOU`](../examples/P_Demo_ModbusRtuMaster_KL6x22B.TcPOU)
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK

```iecst
// 场景：维护老程序——经 KL6041 端子读站地址 2 电表的 6 个保持寄存器（功能码 3）。
PROGRAM P_Demo_ModbusRtuMaster_KL6x22B
VAR
...
- **场景**：维护早期用本 FB 写成的存量工程（KL6031/6041 串口端子 Modbus RTU 主站）。新建项目不应再用它。
- **价值**：保留旧接口让老程序能继续编译运行。
- **替代方案对比**：
  - **新工程**：用 `ModbusRtuMasterV2_KL6x22B`（功能更全）。
  - 其他硬件：旧版 PC 串口用 `ModbusRtuMaster_PcCOM`；旧版 3 字节端子用 `ModbusRtuMaster_KL6x5B`。

## 8. 参考资料

- **PDF**：[TF6255_TC3_Modbus_RTU_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6255_TC3_Modbus_RTU_EN.pdf) §5.1.1.3（[obsolete]）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6255_tc3_modbus_rtu/186542347.html
- **相关 FB / 枚举**：`ModbusRtuMasterV2_KL6x22B`（推荐的新版替代）、`ModbusRtuMaster_PcCOM` / `ModbusRtuMaster_KL6x5B`（同为旧版）、`MODBUS_ERRORS`、`MODBUS_UNITID`
