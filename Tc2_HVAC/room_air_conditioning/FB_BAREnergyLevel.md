...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4684950027.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_BAREnergyLevel.TcPOU`](../examples/P_Demo_FB_BAREnergyLevel.TcPOU) |

---

## 1. 功能简述
房间空气处理 FB（风机盘管 / 节能模式 / 功能选择 / 室内设定）：`FB_BAREnergyLevel` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 8 个 VAR_INPUT 引脚与 5 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `bWindow` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bProtection` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bEconomy` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bPreComfort` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bComfort` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |

#### VAR_OUTPUT 引脚描述

| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `eEnergyLevel` | `E_BAREnergyLevel` | - | 枚举 / 结构（参见 `E_BAREnergyLevel`）。 |
| `bStateProtection` | `BOOL` | - | 状态镜像位（Protection），用于把内部 / 输入状态回显给 HMI。 |
| `bStateEconomy` | `BOOL` | - | 状态镜像位（Economy），用于把内部 / 输入状态回显给 HMI。 |
| `bStatePreComfort` | `BOOL` | - | 状态镜像位（PreComfort），用于把内部 / 输入状态回显给 HMI。 |
| `bStateComfort` | `BOOL` | - | 状态镜像位（Comfort），用于把内部 / 输入状态回显给 HMI。 |

#### VAR_IN_OUT 引脚描述

| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `eCtrlMode` | `E_BAREnergyLevel` | - | 运行模式枚举（参见 `E_HVACCtrlMode`）。BMS = Building Management System（上位机通道），OP = Operator Panel（就地面板）；Auto = 跟随业务逻辑、Manual = 强制 rYManual 设定值。 |

## 3. 行为说明

`FB_BAREnergyLevel` 是 Tc2_HVAC 库 房间空气处理 FB（风机盘管 / 节能模式 / 功能选择 / 室内设定） 子类中的功能块。按 §2 接口定义表列出的引脚顺序，每周期单次调用本 FB；输入信号通过 VAR_INPUT 引脚传入、输出结果通过 VAR_OUTPUT 引脚回读，状态 / 错误位与同库其他 FB 的命名约定保持一致。 上电后 FB 处于禁用态，所有输出回到安全值；`bEnable := TRUE` 触发 FB 进入正常工作。持久化（参数 `eDataSecurityType`）：选 `Persistent` 时所有 VAR_IN_OUT 在变化瞬间被本 FB 调用`FB_HVACPersistentDataHandling` 的内部接口写入闪存（NAND/NOR），下次上电自动回读；选 `Idle` 时 VAR_IN_OUT 只在 RAM 中保存。 `bSetDefault` 上升沿一次性把 VAR_IN_OUT 区所有参数复位为 PDF 列出的默认值，避免下载工程后VAR_IN_OUT 区为 0 / 空导致逻辑误动作。工程现场建议把 HMI 上「恢复默认」按钮接到这个引脚。 每个 PLC 周期都应调用本 FB 一次（不要条件调用、不要在不同任务里调用同一实例）；FB 内部维护状态机 / 积分量 / 时间累积，跳过调用会让计数 / 时序不准。按Tc2_HVAC 全库统一约定，所有输出在 FB 被调用的同一周期内更新，调用方可立即读取输出。
...
> 配套可导入文件：[`examples/P_Demo_FB_BAREnergyLevel.TcPOU`](../examples/P_Demo_FB_BAREnergyLevel.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.5.1.1
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4684950027.html
