...
| Type | `FUNCTION_BLOCK` |
| Category | `HVAC Time Schedule` |
| Source PDF | https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf |
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/9007203939897099.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACSchedulerSpecialPeriods.TcPOU`](../examples/P_Demo_FB_HVACSchedulerSpecialPeriods.TcPOU) |

---

## 1. 功能简述
时间表调度 FB（多通道 / 节假日 / 特殊周期）：`FB_HVACSchedulerSpecialPeriods` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 5 个 VAR_INPUT 引脚与 6 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `bOutput` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bEdgeOn` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bEdgeOff` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `tNextOn` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tNextOff` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACSchedulerSpecialPeriods.TcPOU`](../examples/P_Demo_FB_HVACSchedulerSpecialPeriods.TcPOU)
>
...
- **场景**：HVAC 时间表调度：每天 / 每周 / 节假日 / 特殊周期等多层次时间表，输出当前时段对应的设定值或运行模式。
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.8.7
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/9007203939897099.html
