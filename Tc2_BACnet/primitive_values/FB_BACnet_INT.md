...
| Type | `FUNCTION_BLOCK` |
| Category | `Primitive Value · Signed Integer Value` |
...
| Example | [`examples/P_Demo_FB_BACnet_INT.TcPOU`](../examples/P_Demo_FB_BACnet_INT.TcPOU) |

---

## 1. 功能简述

代表 BACnet 标准里的「Signed Integer Value」对象类型,Primitive Value 系列中的一个 — 用于把单个简单数据类型(整数 / 字符串 / 日期 / 时刻)直接暴露成可被 BMS 读写的 BACnet 对象,无需借助 AV / MV / String 之外的复杂对象类型。Primitive Value 对象本身没有命令优先级、没有 Status_Flags / Reliability,语义就是数据容器。本变体处理 16 位有符号整数(INT, -32768..32767)。注意 BACnet 标准的Signed Integer Value 类型在本库以 `FB_BACnet_INT` 实现(同一文档也提示 PDF §6.1.2 中的Unsigned Integer Value 对应 `FB_BACnet_UINT`,但 §9.14 示例只使用了 INT / UINT 两个 FB 名 — 实际 token 是 `FB_BACnet_INT` 和 `FB_BACnet_UINT`)。PDF §6.1.2 把 10 种 Primitive Value 对象统一表述,§9.14 给完整声明 + 调用示例。
...
| 值 | `nValue` | `INT` | Present_Value 容器(PLC 与 BMS 双向读写) |
...
> 配套可导入文件:[`examples/P_Demo_FB_BACnet_INT.TcPOU`](../examples/P_Demo_FB_BACnet_INT.TcPOU)

```iecst
PROGRAM P_Demo_FB_BACnet_INT
VAR
    fbVal : FB_BACnet_INT;
    val : INT := 15;
END_VAR
...
- **场景**:调度参数表里的最大重启次数整数字段、HVAC 报警计数等需要简单 INT 容器的场景。
