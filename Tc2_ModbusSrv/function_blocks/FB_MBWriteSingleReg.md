...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192760075.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBWriteSingleReg.TcPOU`](../examples/P_Demo_FB_MBWriteSingleReg.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 6（Write Single Register）的客户端功能块：向远端 Modbus 设备写单个输出寄存器，采用 16 位访问。`nValue` 为要写入的数据字。`bExecute` 上升沿触发一次写入。
...
**触发与状态机**：`bExecute` 上升沿触发一次写单寄存器操作。FB 通过 ADS 异步把请求转交给 TwinCAT Modbus TCP 服务组件，由它向 `sIPAddr` 指定的远端 Modbus 设备发出 TCP（端口 502，面向连接）请求。请求发出后 `bBUSY := TRUE`；收到对端响应或超时后，`bBUSY := FALSE`，并据结果置 `bError` 与 `nErrId`。`bExecute` 在 `bBUSY` 期间保持高电平不会重复触发，必须等一次执行结束、再来一个上升沿才会重发。
...
> 配套可导入文件：[`examples/P_Demo_FB_MBWriteSingleReg.TcPOU`](../examples/P_Demo_FB_MBWriteSingleReg.TcPOU)
>
...
- **场景**：向变频器/温控表写单个设定值，例如改写一台变频器的目标频率寄存器，或给温控表下发新的设定温度。
- **价值**：单点写寄存器用功能码 6；本 FB 一次调用即可把一个 `WORD` 写到指定寄存器，省去报文构造。
- **替代方案对比**：替代是写多寄存器（功能码 16）写 1 个字——可行但更重；或裸 socket。单点设定本 FB 最简。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.6
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192760075.html
- **相关**：`FB_MBUdpWriteSingleReg`（UDP 版）、`FB_MBWriteRegs`（批量写寄存器）、`stLibVersion_Tc2_ModbusSrv`（库版本）
