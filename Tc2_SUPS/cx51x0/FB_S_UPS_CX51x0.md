...
`FB_S_UPS_CX51x0` 用于 **CX51x0** 平台带 1-second UPS 设备从 PLC 控制 UPS。断电时根据 `eUpsMode` 决定保存持久化数据或/和执行 quick shutdown。**默认输入值不应改动**。
...
VAR_INPUT
    ...
    iUPSPort        : UINT; (* Port for reading Power State of UPS *)
...
| `iUPSPort` | `UINT` | 读 UPS 状态的端口（默认 16#588） |
...
VAR
    fbFB_S_UPS_CX51x0 : FB_S_UPS_CX51x0;
