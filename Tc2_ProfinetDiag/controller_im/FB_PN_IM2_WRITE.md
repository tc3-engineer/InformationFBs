...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_profinetdiag/14965877899.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_PN_IM2_WRITE.TcPOU`](../examples/P_Demo_FB_PN_IM2_WRITE.TcPOU) |

---

## 1. 功能简述

PROFINET 控制器用本功能块把全部 I&M2 数据写入指定 `PORT` 引用的设备。I&M2 帧结构对应 PROFINET 标准索引 `0xAFF2`。通过输入 `str_Date`（`TIMESTRUCT`，格式 `<YYYY-MM-DD HH:MM>`）把安装日期写入设备。
...
- 写入前先填好 `str_Date` 的各字段（年/月/日/时/分）。

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_FB_PN_IM2_WRITE.TcPOU`](../examples/P_Demo_FB_PN_IM2_WRITE.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK

```iecst
PROGRAM P_Demo_FB_PN_IM2_WRITE
VAR
...
- **PDF**：[`TwinCAT_3_PLC_Lib_Tc2_ProfinetDiag_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_ProfinetDiag_EN.pdf) 第 3.1.2.5 节
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_profinetdiag/14965877899.html
- **相关 FB / FC**：`FB_PN_IM2_READ`（读 I&M2）、`str_IM_0xAFF2`（数据结构）
