...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/5855743883.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_PT1.TcPOU`](../examples/P_Demo_FB_FTR_PT1.TcPOU) |

---

## 1. 功能简述

实现一个一阶延迟环节（PT1 element），拉普拉斯域传递函数 `G(s) = Kp / (1 + T1·s)`。由配置结构 `ST_FTR_PT1` 给定增益 `fKp`、时间常数 `fT1` 与采样率。PT1 与一阶 Butterworth 低通等价，是控制工程里最基础的平滑/惯性环节。

> 类别归属：一阶延迟环节（PT1）。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_PT1` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_PT1(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
对阶跃输入，输出按指数 `1 − e^(−t/T1)` 上升，经过约 `3·T1` 达到稳态值的 95%、`5·T1` 达到 99%。时间常数 `fT1` 越大平滑越强、响应越慢。增益 `fKp` 决定稳态放大倍数（`fKp = 1` 时输出稳态等于输入）。一个 PT1 元件可与一阶 Butterworth 低通相互等价转换，只是参数表示方式不同（PT1 用时间常数，Butterworth 用截止频率）。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_PT1(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_PT1.TcPOU`](../examples/P_Demo_FB_FTR_PT1.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：一路温度测量值（如 EL3201 PT100）读数带高频噪声，用一个 T1 = 0.5 s 的 PT1 做平滑，让闭环温控的反馈量平顺、避免控制器对噪声过度反应。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_PT1，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：PT1 是工程上最常用的惯性平滑环节，用时间常数表达比截止频率更贴近控制工程师的直觉；本 FB 自动处理离散化（双线性变换），无需手算 z 域系数。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_PT1;                  // 被演示的滤波器实例
    stParams      : ST_FTR_PT1 := (fKp := 1, fT1 := 0.5, fSamplingRate := 1000, nOversamples := 1, nChannels := 1);
...
- **场景**：一路温度测量值（如 EL3201 PT100）读数带高频噪声，用一个 T1 = 0.5 s 的 PT1 做平滑，让闭环温控的反馈量平顺、避免控制器对噪声过度反应。
- **价值**：PT1 是工程上最常用的惯性平滑环节，用时间常数表达比截止频率更贴近控制工程师的直觉；本 FB 自动处理离散化（双线性变换），无需手算 z 域系数。
- **替代方案对比**：自己写 `y := y + (x - y) * dt / T1`：原理对，但缺少多通道、过采样、参数校验，且离散化精度不如库内双线性变换；需要更陡的滚降时用更高阶的 `FB_FTR_PT2/PT3/PTn` 或 `FB_FTR_IIRSpec`。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.5（功能块）、§5.2.1.5（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/5855743883.html
