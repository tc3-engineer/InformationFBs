...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/9007199892817803.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_LicFileCopyFromDongle.TcPOU`](../examples/P_Demo_FB_LicFileCopyFromDongle.TcPOU) |
...
| `nPortSrc` | `UINT` | - | 无符号整数输入：`nPortSrc`。 |
| `sNetIdDest` | `T_AmsNetId` | - | 参数 `sNetIdDest`（类型 `T_AmsNetId`）。⚠️ PDF 未详述含义，请按 §3 行为说明使用。 |
| `sFileNameSrc` | `STRING` | - | 字符串输入：`sFileNameSrc`。 |
| `sFilePathNameDest` | `T_MaxString` | - | 参数 `sFilePathNameDest`（类型 `T_MaxString`）。⚠️ PDF 未详述含义，请按 §3 行为说明使用。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_LicFileCopyFromDongle.TcPOU`](../examples/P_Demo_FB_LicFileCopyFromDongle.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）。
>
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf) §3.40
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/9007199892817803.html
- **相关 FB**：`FB_LicFileCopyToDongle`
