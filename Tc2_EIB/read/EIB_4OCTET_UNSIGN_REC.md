...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187738891.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_4OCTET_UNSIGN_REC.TcPOU`](../examples/P_Demo_EIB_4OCTET_UNSIGN_REC.TcPOU) |

---

## 1. 功能简述

接收 EIB 网络上指定组地址的 **4 字节无符号整数 telegram**，对应 KNX DPT 12.xxx。解码到 IEC `UDINT`（0..4294967295）。
...
| `uiData` | `UDINT` | 收到的负载解码为 IEC `UDINT`（0..4294967295） |
...
**数据宽度匹配**：发送端 telegram 的负载长度必须与本 FB 类型对应（本 FB 需要 4-byte unsigned int telegram）。类型不匹配的 telegram 会被 KL6301 丢弃（`WRONG_EIB_DATA_LEN`），本 FB 永远收不到。
...
> 配套可导入文件：[`examples/P_Demo_EIB_4OCTET_UNSIGN_REC.TcPOU`](../examples/P_Demo_EIB_4OCTET_UNSIGN_REC.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）

> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_EIB_4OCTET_UNSIGN_REC
VAR
    fbEIB_4OCTET_UNSIGN_REC        : EIB_4OCTET_UNSIGN_REC;
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.4.7
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187738891.html
- **相关**：`EIB_4OCTET_SIGN_REC`、`KL6301`
