...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4684924299.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACMotor3Speed.TcPOU`](../examples/P_Demo_FB_HVACMotor3Speed.TcPOU) |

---

## 1. 功能简述
执行器控制 FB（阀门 / 风阀 / 水泵 / 电机）：`FB_HVACMotor3Speed` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 18 个 VAR_INPUT 引脚与 20 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `bAutoSpeed1` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bAutoSpeed2` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bAutoSpeed3` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `eCtrlModeActuator` | `E_HVACActuatorMode` | - | 运行模式枚举（参见 `E_HVAC*ActuatorMode`）：Auto_BMS / Manual_BMS / Auto_OP / Manual_OP 等；BMS 来自上位机命令通道，OP 来自就地操作面板，两通道独立。 |
| `bMotorProtecSpeed1` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bMotorProtecSpeed2` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bMotorProtecSpeed3` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bFeedbContactorSpeed1` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bFeedbContactorSpeed2` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bFeedbContactorSpeed3` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bFeedbProcess` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bRepairSwitch` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `bMotorSpeed1` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bMotorSpeed2` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bMotorSpeed3` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `wState` | `WORD` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `bStateAutoSpeed1` | `BOOL` | - | 状态镜像位（AutoSpeed1），用于把内部 / 输入状态回显给 HMI。 |
| `bStateAutoSpeed2` | `BOOL` | - | 状态镜像位（AutoSpeed2），用于把内部 / 输入状态回显给 HMI。 |
| `bStateAutoSpeed3` | `BOOL` | - | 状态镜像位（AutoSpeed3），用于把内部 / 输入状态回显给 HMI。 |
| `bStateRepairSwitch` | `BOOL` | - | 状态镜像位（RepairSwitch），用于把内部 / 输入状态回显给 HMI。 |
| `bStateManSwitch` | `BOOL` | - | 状态镜像位（ManSwitch），用于把内部 / 输入状态回显给 HMI。 |
| `eStateModeActuator` | `E_HVACActuatorMode` | - | 当前实际运行模式枚举回读（ModeActuator）。 |
| `bErrorGeneral` | `BOOL` | - | 错误指示位（General）。`bReset` 上升沿清错。 |
| `wError` | `WORD` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `bErrorMotorprotecSpeed1` | `BOOL` | - | 错误指示位（MotorprotecSpeed1）。`bReset` 上升沿清错。 |
| `bErrorMotorprotecSpeed2` | `BOOL` | - | 错误指示位（MotorprotecSpeed2）。`bReset` 上升沿清错。 |
| `bErrorMotorprotecSpeed3` | `BOOL` | - | 错误指示位（MotorprotecSpeed3）。`bReset` 上升沿清错。 |
| `bErrorFeedbContactorSpeed1` | `BOOL` | - | 错误指示位（FeedbContactorSpeed1）。`bReset` 上升沿清错。 |
| `bErrorFeedbContactorSpeed2` | `BOOL` | - | 错误指示位（FeedbContactorSpeed2）。`bReset` 上升沿清错。 |
| `bErrorFeedbContactorSpeed3` | `BOOL` | - | 错误指示位（FeedbContactorSpeed3）。`bReset` 上升沿清错。 |
| `bErrorFeedbProcess` | `BOOL` | - | 错误指示位（FeedbProcess）。`bReset` 上升沿清错。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `tStartDelay` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tStopDelay` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelayFeedbContactorSpeed1` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelayFeedbContactorSpeed2` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelayFeedbContactorSpeed3` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tFeedbProcessTimer` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelayFeedbProcess` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelaySpeed1` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelaySpeed2` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelaySpeed1ToSpeed2ToSpeed3` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelaySpeed2ToSpeed1` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tDelaySpeed3ToSpeed2` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |

## 3. 行为说明

`FB_HVACMotor3Speed` 是 Tc2_HVAC 库 执行器控制 FB（阀门 / 风阀 / 水泵 / 电机） 子类中的功能块。按 §2 接口定义表列出的引脚顺序，每周期单次调用本 FB；输入信号通过 VAR_INPUT 引脚传入、输出结果通过 VAR_OUTPUT 引脚回读，状态 / 错误位与同库其他 FB 的命名约定保持一致。 上电后 FB 处于禁用态，所有输出回到安全值；`bEnable := TRUE` 触发 FB 进入正常工作。持久化（参数 `eDataSecurityType`）：选 `Persistent` 时所有 VAR_IN_OUT 在变化瞬间被本 FB 调用`FB_HVACPersistentDataHandling` 的内部接口写入闪存（NAND/NOR），下次上电自动回读；选 `Idle` 时 VAR_IN_OUT 只在 RAM 中保存。 错误处理：`bError*` 系列输出一旦置 TRUE 则锁存（不会自动复位），必须在故障原因消除后给 `bReset` 一次FALSE→TRUE 上升沿才能清错。`bInvalidParameter` 表示 VAR_IN_OUT 参数超出 PDF 列出的合法范围（量程 / 时间 / 长度），排错时优先检查上述参数。 `bSetDefault` 上升沿一次性把 VAR_IN_OUT 区所有参数复位为 PDF 列出的默认值，避免下载工程后VAR_IN_OUT 区为 0 / 空导致逻辑误动作。工程现场建议把 HMI 上「恢复默认」按钮接到这个引脚。 每个 PLC 周期都应调用本 FB 一次（不要条件调用、不要在不同任务里调用同一实例）；FB 内部维护状态机 / 积分量 / 时间累积，跳过调用会让计数 / 时序不准。按Tc2_HVAC 全库统一约定，所有输出在 FB 被调用的同一周期内更新，调用方可立即读取输出。
...
| `bErrorMotorprotecSpeed1` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorMotorprotecSpeed2` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorMotorprotecSpeed3` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorFeedbContactorSpeed1` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorFeedbContactorSpeed2` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorFeedbContactorSpeed3` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorFeedbProcess` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACMotor3Speed.TcPOU`](../examples/P_Demo_FB_HVACMotor3Speed.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.2.7
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4684924299.html
