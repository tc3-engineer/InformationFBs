...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11313590411.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_PTt.TcPOU`](../examples/P_Demo_FB_FTR_PTt.TcPOU) |

---

## 1. 功能简述

实现一个纯延迟环节（dead time element，PTt element），拉普拉斯域传递函数 `G(s) = Kp · e^(−Tt·s)`。由配置结构 `ST_FTR_PTt` 给定增益 `fKp`、死区时间 `fTt` 与采样率。效果是把输入信号原样延迟 `fTt` 秒后再输出（乘以增益），不改变波形。

> 类别归属：纯延迟（死区时间）环节。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_PTt` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_PTt(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
死区时间 `fTt` 必须是采样周期 `1/fSamplingRate` 的整数倍（否则配置报错），因为离散实现是把输入压入长度 = `fTt·fSamplingRate` 的移位缓冲。注意本 FB 的 `ST_FTR_PTt` **没有** `pInitialValues`/`nInitialValuesSize` 字段（结构里只有 `fKp`/`fTt`/`fSamplingRate`/`nOversamples`/`nChannels`）。纯延迟常用来模拟传输滞后（皮带输送、管道流动）或在控制中做时间对齐。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_PTt(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_PTt.TcPOU`](../examples/P_Demo_FB_FTR_PTt.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：皮带秤称重点与执行机构相距一段皮带，物料从称重到落料有固定输送延迟；用 PTt 把称重信号延迟相同时间，使其与下游动作在时间上对齐。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_PTt，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：把『把信号延迟固定时间』封装成一行配置，自动按采样周期对齐缓冲长度，免去手写移位寄存器。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_PTt;                  // 被演示的滤波器实例
    stParams      : ST_FTR_PTt := (fKp := 1, fTt := 0.1, fSamplingRate := 1000, nOversamples := 1, nChannels := 1);
...
- **场景**：皮带秤称重点与执行机构相距一段皮带，物料从称重到落料有固定输送延迟；用 PTt 把称重信号延迟相同时间，使其与下游动作在时间上对齐。
- **价值**：把『把信号延迟固定时间』封装成一行配置，自动按采样周期对齐缓冲长度，免去手写移位寄存器。
- **替代方案对比**：自己开数组做移位缓冲：要算缓冲长度、处理回绕；本 FB 自动校验 `fTt` 是采样周期整数倍并管理缓冲。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.12（功能块）、§5.2.1.12（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11313590411.html
