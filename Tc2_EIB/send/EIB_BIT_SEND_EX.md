...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187780363.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_BIT_SEND_EX.TcPOU`](../examples/P_Demo_EIB_BIT_SEND_EX.TcPOU) |

---

## 1. 功能简述

**`EIB_BIT_SEND` 的扩展版本**：发送 DPT 1.xxx (1-bit) telegram，支持 Manual / Polling / OnChange / OnChangePolling 4 种模式。
...
| `CyclePolling` | `TIME` | `t#10s` | Polling 模式下的发送周期。**下限 200 ms**（库强制） |
...
> 配套可导入文件：[`examples/P_Demo_EIB_BIT_SEND_EX.TcPOU`](../examples/P_Demo_EIB_BIT_SEND_EX.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）

> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_EIB_BIT_SEND_EX
VAR
    fb       : EIB_BIT_SEND_EX;
    stEibRec : EIB_REC;
    stGroup  : EIB_GROUP_ADDR := (MAIN := 1, SUB_MAIN := 1, NUMBER := 50);
    data     : BOOL := FALSE;
END_VAR
fb(bStart := TRUE, iMode := 3, CyclePolling := T#5S, MinSendTime := T#500MS,
   Group_Address := stGroup, bData := data, str_Rec := stEibRec,
   bEnableReadReq := TRUE);
```

## 7. 业务场景与实际价值

- **场景**：PLC 开关命令定时心跳 + 即时变化 + 响应主动查询
- **价值**：BMS 工程的实际首选方式
- **替代方案对比**：
  - `EIB_BIT_SEND`：只变化发，无周期
  - 本 _EX：BMS 工程**首选**

## 8. 参考资料

- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.23
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187780363.html
- **相关**：`EIB_BIT_SEND`（同库 §4.2.5.22，简化版）、对应接收 FB
