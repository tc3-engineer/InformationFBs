...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192772235.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBUdpReadRegs.TcPOU`](../examples/P_Demo_FB_MBUdpReadRegs.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 3（Read Holding Registers）的客户端功能块：从远端 Modbus 设备读取 1 到 128 个输出寄存器（16 位）。每个寄存器低字节在前、高字节在后写入缓冲区。`bExecute` 上升沿触发一次读取。 本块是 UDP 变体：底层用无连接的 Modbus/UDP 报文传输，接口与同名 TCP 版 `FB_MBReadRegs` 完全一致，区别仅在传输层（不建立 TCP 连接，不保证可靠交付）。
...
| `nQuantity` | `WORD` | `-` | 要读取的输出寄存器（数据字）个数。值为 0 不允许 |
| `nMBAddr` | `WORD` | `-` | 要读取的输出寄存器起始地址（字偏移） |
...
**角色定位**：本库的 FB_MB* 系列让 PLC 充当 Modbus **主站（master/client）**，主动去读写远端设备的保持寄存器（输出寄存器，可读写）。它与“TwinCAT 自身作为 Modbus 从站被外部 SCADA 访问”是两回事——后者由 TF6250 服务端 + 配置器 + `mb_Input_Coils` 等 GVL 数组 + ADS 映射完成，不经过本 FB。
...
> 配套可导入文件：[`examples/P_Demo_FB_MBUdpReadRegs.TcPOU`](../examples/P_Demo_FB_MBUdpReadRegs.TcPOU)
>
...
- **替代方案对比**：替代是裸 socket 自写协议，或用厂商专有协议网关。对标准 Modbus 设备，本 FB 通用且零额外硬件。 与 TCP 版 `FB_MBReadRegs` 的取舍：需要可靠交付选 TCP，需要低延迟且可容忍丢包选本 UDP 版。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.11.3
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192772235.html
- **相关**：`FB_MBReadRegs`（TCP 版）、`FB_MBReadRegs` / `FB_MBReadInputRegs`（读寄存器）、`stLibVersion_Tc2_ModbusSrv`（库版本）
