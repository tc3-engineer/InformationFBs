...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf5100_tc3_nc_i/3284457355.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_ItpGetCyclicLrealOffsets.TcPOU`](../examples/P_Demo_ItpGetCyclicLrealOffsets.TcPOU) |

---

## 1. 功能简述

`ItpGetCyclicLrealOffsets` 读回当前 NC 端为 4 个 LREAL Cyclic Param 配置的字节偏移量。Cyclic Param 是 NC 把变量直接写进 NCTOPLC 通道接口的高速通道；先用 `ItpSetCyclicLrealOffsets` 在 NC 端配置好偏移，PLC 就能用 `ItpReadCyclicLRealParam[1..4]` 直接读到。
...
| `nIndexOffsetParam1` | `UDINT` | 第 1 个 cyclic 参数对应的 NC 内部状态字段字节偏移；填 0 时禁用此槽，整个 cyclic 参数功能也会关闭（PDF 原文：only active if nIndexOffsetParam1 ≠ 0） |
| `nIndexOffsetParam2` | `UDINT` | 第 2 个 cyclic 参数对应的字节偏移；填 0 时不启用 |
| `nIndexOffsetParam3` | `UDINT` | 第 3 个 cyclic 参数对应的字节偏移；填 0 时不启用 |
| `nIndexOffsetParam4` | `UDINT` | 第 4 个 cyclic 参数对应的字节偏移；填 0 时不启用 |
...
`ItpGetCyclicLrealOffsets` 走 ADS 同步调用流程：在 `bExecute` 检测到上升沿时把命令打包发给 NCI、进入 `bBusy = TRUE` 状态等回包；NC 端接受后 `bBusy = FALSE`，若执行失败则 `bErr = TRUE` 并把错误号写入 `nErrId`。注意 PDF 原文明确指出 `bBusy` 监视的是 NC 端『接受』命令的时间，不是命令真正执行完成的时间——也就是说 `bBusy = FALSE` 不等于动作做完，只是 ADS 调用收到了应答。
...
> 配套可导入文件：[`examples/P_Demo_ItpGetCyclicLrealOffsets.TcPOU`](../examples/P_Demo_ItpGetCyclicLrealOffsets.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK

```iecst
fbItpGetCyclicLrealOffsets(
    bExecute           := rtTrig.Q,
    tTimeOut           := T#2S,
    ...
    bErr               => bErr_out,
    nErrId             => nErrId_out,
    nIndexOffsetParam1 => nIndexOffsetParam1_out,
    nIndexOffsetParam2 => nIndexOffsetParam2_out,
    nIndexOffsetParam3 => nIndexOffsetParam3_out,
    nIndexOffsetParam4 => nIndexOffsetParam4_out
);
```
...
- **PDF**：[TF5100_TC3_NC_I_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF5100_TC3_NC_I_EN.pdf) §7.1.2.10
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf5100_tc3_nc_i/3284457355.html
- **相关 FB / FC**：见 §3 行为说明
