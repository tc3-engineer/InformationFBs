...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc3_eventlogger/5051503499.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_OnMessageSent.TcPOU`](../examples/P_Demo_OnMessageSent.TcPOU) |
...
子类 OVERRIDE 本方法即可在每条 message 发送时执行业务逻辑——如审计日志同步、事件转发到 OPC UA / MQTT / 第三方 SCADA 等。
...
本方法是事件驱动回调（callback），由 EventLogger 在 message 事件被发送时 调用。调用上下文是 PLC 任务（调用 listener.Execute() 的任务），因此回调里的代码占用 PLC 周期时间。
...
> 配套可导入文件：[`examples/P_Demo_OnMessageSent.TcPOU`](../examples/P_Demo_OnMessageSent.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
MES 集成：每条 message 实时同步到 InfluxDB 时序数据库以做长期审计
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc3_EventLogger_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_EventLogger_EN.pdf) §3.5.6
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc3_eventlogger/5051503499.html
