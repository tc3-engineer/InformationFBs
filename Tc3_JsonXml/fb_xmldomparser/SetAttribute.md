...
| Status | `verified` |
| Example | [`examples/P_Demo_FB_XmlDomParser_SetAttribute.TcPOU`](../examples/P_Demo_FB_XmlDomParser_SetAttribute.TcPOU) |

---

## 1. 功能简述

把 JSON 节点的 Attribute 属性设置为传入值。返回更新后的 `SJsonValue` 句柄。
...
> 配套可导入文件：[`examples/P_Demo_FB_XmlDomParser_SetAttribute.TcPOU`](../examples/P_Demo_FB_XmlDomParser_SetAttribute.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **场景**：修改已有 JSON 节点的 Attribute 类型值（如更新缓存的实时数据字段）。
- **价值**：DOM 树本地修改不重新构造整个文档，比 GetDocument + 字符串替换效率高。
- **替代方案对比**：重新组建整个 JSON → 大文档时性能差；字符串 Find + Replace → 同名字段容易误改。

## 8. 参考资料

- **PDF**：[`TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf) §4.7.81
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc3_jsonxml/5512100491.html
- **相关 FB / FC**：`FB_XmlDomParser`, `AddAttributeMember`, `GetAttribute`
