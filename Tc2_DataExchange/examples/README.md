...
2. 把刚导入的 PROGRAM 加到 PlcTask 调用列表（`MAIN` 或新建任务）
   - 或在 `MAIN` 里加一行 `P_Demo_FB_CheckWatchdog();`
...
- 引用 `Tc2_DataExchange` 等 TwinCAT 内置/附加库的 derived 类型不可移植到非 Beckhoff IDE
...
| 编译报 "FB_CheckWatchdog not defined" | PLC 项目缺 `Tc2_DataExchange` 引用 |
