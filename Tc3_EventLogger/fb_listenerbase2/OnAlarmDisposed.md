...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc3_eventlogger/5051475339.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_OnAlarmDisposed.TcPOU`](../examples/P_Demo_OnAlarmDisposed.TcPOU) |
...
通常发生在 `FB_TcAlarm.Clear(bRemove := TRUE)` 或 `FB_TcAlarm.Release()` 之后。子类 OVERRIDE 用于资源清理或追溯日志。
...
本方法是事件驱动回调（callback），由 EventLogger 在 alarm 实例从活动表移除时 调用。调用上下文是 PLC 任务（调用 listener.Execute() 的任务），因此回调里的代码占用 PLC 周期时间。
...
> 配套可导入文件：[`examples/P_Demo_OnAlarmDisposed.TcPOU`](../examples/P_Demo_OnAlarmDisposed.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
动态报警场景：alarm 实例销毁时清理 HMI 端的对应展示卡片
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc3_EventLogger_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_EventLogger_EN.pdf) §3.5.4
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc3_eventlogger/5051475339.html
