...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187758859.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_2OCTET_UNSIGN_SEND.TcPOU`](../examples/P_Demo_EIB_2OCTET_UNSIGN_SEND.TcPOU) |

---

## 1. 功能简述

**发送 DPT 7.xxx (2-byte unsigned int) telegram**：把 IEC `UINT` 值经库编码成 EIB 标准格式，发送到指定 `Group_Address`。**只在 `uiData` 值变化时下发**——周期重发要用 `EIB_2OCTET_UNSIGN_SEND_EX`。
...
**触发方式**：每周期调用本 FB；**只要数据值（uiData）发生变化**，库就把新值编码成 EIB telegram 并下发到 EIB 总线。**初次调用**也会发一次（首次值通过）。
...
> 配套可导入文件：[`examples/P_Demo_EIB_2OCTET_UNSIGN_SEND.TcPOU`](../examples/P_Demo_EIB_2OCTET_UNSIGN_SEND.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）

> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_EIB_2OCTET_UNSIGN_SEND
VAR
    fb       : EIB_2OCTET_UNSIGN_SEND;
    stEibRec : EIB_REC;
    stGroup  : EIB_GROUP_ADDR := (MAIN := 1, SUB_MAIN := 4, NUMBER := 12);
    data     : UINT := UINT#0;
END_VAR
fb(Group_Address := stGroup, uiData := data, str_Rec := stEibRec);
```

## 7. 业务场景与实际价值

- **场景**：PLC 亮度值（0..65535）发送到 BMS
- **价值**：DPT DPT 7.xxx 标准发送方式
- **替代方案对比**：
  - `EIB_2OCTET_UNSIGN_SEND_EX`：周期发 + read 响应
  - `EIB_ALL_DATA_TYPES_SEND`：raw byte 模式
  - 本 FB：简单变化发送的标准选择

## 8. 参考资料

- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.5
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187758859.html
- **相关**：`EIB_2OCTET_UNSIGN_SEND_EX`（同库 §4.2.5.6）、对应接收 FB
