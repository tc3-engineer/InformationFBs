...
- 引用 Tc2_Standard 等 TwinCAT 内置库的 derived 类型不可移植到非 Beckhoff IDE
- 引用 Tc2_MC2 等附加库时，导入前必须先在目标项目中已添加该库引用，否则 derived 类型解析失败
...
| 编译报 "RS not defined" | PLC 项目缺 `Tc2_Standard` 引用（默认 Standard 项目应有） |
