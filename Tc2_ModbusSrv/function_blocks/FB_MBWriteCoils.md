...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192761611.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBWriteCoils.TcPOU`](../examples/P_Demo_FB_MBWriteCoils.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 15（Write Multiple Coils）的客户端功能块：向远端 Modbus 设备写 1 到 2048 个数字量输出（线圈）。源缓冲区里每个位对应一个线圈。`bExecute` 上升沿触发一次写入。
...
**触发与状态机**：`bExecute` 上升沿触发一次批量写操作。FB 通过 ADS 异步把请求转交给 TwinCAT Modbus TCP 服务组件，由它向 `sIPAddr` 指定的远端 Modbus 设备发出 TCP（端口 502，面向连接）请求。请求发出后 `bBUSY := TRUE`；收到对端响应或超时后，`bBUSY := FALSE`，并据结果置 `bError` 与 `nErrId`。`bExecute` 在 `bBUSY` 期间保持高电平不会重复触发，必须等一次执行结束、再来一个上升沿才会重发。
...
> 配套可导入文件：[`examples/P_Demo_FB_MBWriteCoils.TcPOU`](../examples/P_Demo_FB_MBWriteCoils.TcPOU)
>
...
- **场景**：一次性下发一批继电器/输出状态，例如远端 IO 站的 10 路输出按一个 `BYTE` 模式批量置位，用于灯光/电磁阀阵列控制。
- **价值**：批量写线圈用功能码 15，比逐个功能码 5 少很多报文往返；本 FB 按位打包一次写出。
- **替代方案对比**：替代是循环调用 WriteSingleCoil——报文多、延迟高；或裸 socket。批量输出本 FB 高效。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.7
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192761611.html
- **相关**：`FB_MBUdpWriteCoils`（UDP 版）、`FB_MBReadRegs`（先读后用）、`stLibVersion_Tc2_ModbusSrv`（库版本）
