...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc3_mc2_advancedhoming/427766923.html |
| Verified | 2026-05-25 ✅ |
| InfoSys-checked | ✅ 2026-05-25 |
| Status | `verified` |
| Example | [`examples/P_Demo_MC_StepBlockLagBasedDetection.TcPOU`](../examples/P_Demo_MC_StepBlockLagBasedDetection.TcPOU) |

---

## 1. 功能简述

**主动归零 step 函数：撞机械止挡（跟随误差判据，Detection 版）**。搜索逻辑与 `MC_StepBlockLagBased` 相同（限矩顶止挡，跟随误差超 `LagLimit` + 速度持续低于限值双判据），但本 Detection 版**不改写轴位置**，而是把检测到止挡那一刻的轴位置通过 `RecordedPosition` 返回给用户。
...
| `LagLimit` | `LREAL` | 位置跟随误差阈值；跟随误差超过此值即判定为顶到机械止挡 |
...
`Execute` 上升沿启动搜索。本 FB 驱动轴朝 `Direction` 方向运动去顶一个机械固定止挡（撞块），为避免损坏机械，运动以 `TorqueLimit` 限矩进行。成功条件由两部分组成：位置跟随误差超过 `LagLimit`，且实际速度在 `DetectionVelocityTime` 内持续低于 `DetectionVelocityLimit`。区别在于按 PDF 第 3.3.6 节，本 Detection 版**不会把轴位置改写**，而是把检测到止挡那一刻的轴位置写入输出 `RecordedPosition` 返回。⚠️ 提示：PDF 第 3.3.6 节正文的 `VAR_INPUT` 代码块多印了一个 `SetPosition : LREAL`（本文档 §2 逐字照搬保留），但本 FB 是 Detection 版、其接口图不含 `SetPosition`，该入口对结果无效（Detection 版不改写轴位置），实际位置仅经 `RecordedPosition` 返回。满足上述两个条件即判定已顶到止挡。`Velocity` / `Acceleration` / `Deceleration` / `Jerk` 定义搜索运动动力学；超过 `TimeLimit` 或相对起点移动超 `DistanceLimit` 则 `CommandAborted` 置位。`Options`（`ST_Home_Options3`）的 `InstantLagReduction` 用于顶到“软”止挡突停后瞬时消除跟随误差。本 FB 在 `Homing` 状态执行、结束后保持 `Homing`，需配 `MC_FinishHoming` / `MC_AbortHoming` 收尾；`Parameter`（`MC_HomingParameter`）在序列各 FB 间逐级传递。
...
- `LagLimit` 要明显大于正常运动的跟随误差波动，否则会误判 ⚠️。

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_MC_StepBlockLagBasedDetection.TcPOU`](../examples/P_Demo_MC_StepBlockLagBasedDetection.TcPOU)

详见上述 XML 文件，内含场景 / 价值 / 验证步骤注释，可右键 PLC 项目下 POUs 文件夹 → Add → Existing Item 导入后编译运行。

## 7. 业务场景与实际价值

- **场景**：无转矩反馈的驱动，需要先测出机械止挡实际位置（磨损 / 偏差监测）再决定置零策略。
- **价值**：跟随误差判据 + 测量返回，既不依赖转矩信号又把置零策略交给上层。
- **替代方案对比**：`MC_StepBlockLagBased`（顶到即置零）；`MC_StepBlockDetection`（同为 Detection 但用转矩判据）。本 FB 用跟随误差判据并只返回位置。

## 8. 参考资料

- **PDF**：[TwinCAT_3_PLC_Lib_Tc3_MC2_AdvancedHoming_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_MC2_AdvancedHoming_EN.pdf) 第 3.3.6 节（Version 1.7.7）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc3_mc2_advancedhoming/427766923.html
- **数据类型（本库共用）**：
