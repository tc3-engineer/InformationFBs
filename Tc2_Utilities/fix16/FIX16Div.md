...
两个有符号 16-bit 定点数相除（截断对齐到较低分辨率），返回 T_FIX16。
## 2. 接口定义
...
- 调用 `FIX16Div(a, b)`，返回 `T_FIX16` 类型结果。
- 期望：`-2.25（=-22.5/10.0）`
...
a := LREAL_TO_FIX16(-22.5, 8); b := LREAL_TO_FIX16(10.0, 8);
IF bRun THEN
    bResult := FIX16Div(a, b);
    bRun := FALSE;
END_IF;
```
