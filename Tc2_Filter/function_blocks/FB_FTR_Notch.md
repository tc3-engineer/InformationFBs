...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11004515595.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_Notch.TcPOU`](../examples/P_Demo_FB_FTR_Notch.TcPOU) |

---

## 1. 功能简述

实现一个窄带宽的带阻滤波器（陷波器 / notch filter），用于精确压制某一个干扰频率。由配置结构 `ST_FTR_Notch` 给定陷波频率 `fNotchFrequency`、品质因数 `fQ` 与采样率。典型用途是滤除工频（50/60 Hz）或某个机械共振频率。

> 类别归属：窄带带阻（陷波）滤波器。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_Notch` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_Notch(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
品质因数 `fQ = fNotchFrequency / 带宽`：`fQ` 越大，陷波越窄越尖锐（只压目标频率、对邻近频率影响小）；`fQ` 越小则带宽越宽。陷波频率必须满足 `0 < fNotchFrequency < fSamplingRate/2`（奈奎斯特限制）。PDF 配置示例：要陷波 100 Hz、单边带宽 10 Hz（即 -3 dB 落在 90 Hz 与 110 Hz），设 `fNotchFrequency := 100; fQ := 10;`（带宽 B = 100/10 = 10 Hz）。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_Notch(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_Notch.TcPOU`](../examples/P_Demo_FB_FTR_Notch.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：一路传感器信号上叠加了 50 Hz 工频干扰（电源耦合进来的），用陷波器精确滤掉 50 Hz，同时尽量不动其它频率的有用信号。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_Notch，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：只压制一个窄频带，对有用信号的其它频率几乎无影响——这是低通/带通做不到的精确抑制。用 `fQ` 直接调节陷波宽窄，参数直观。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_Notch;                  // 被演示的滤波器实例
    stParams      : ST_FTR_Notch := (fNotchFrequency := 50, fQ := 10, fSamplingRate := 1000, nOversamples := 1, nChannels := 1);
...
- **场景**：一路传感器信号上叠加了 50 Hz 工频干扰（电源耦合进来的），用陷波器精确滤掉 50 Hz，同时尽量不动其它频率的有用信号。
- **价值**：只压制一个窄频带，对有用信号的其它频率几乎无影响——这是低通/带通做不到的精确抑制。用 `fQ` 直接调节陷波宽窄，参数直观。
- **替代方案对比**：用带阻型 `FB_FTR_IIRSpec`（`eBandStop`）：能做带阻但参数化方式不同、不便直接给品质因数；窄带工频/共振抑制场景用本 FB 最直接。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.9（功能块）、§5.2.1.9（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11004515595.html
