...
| Example | [`examples/P_Demo_FB_HVACFreezeProtectionHeater.TcPOU`](../examples/P_Demo_FB_HVACFreezeProtectionHeater.TcPOU) |

---

## 1. 功能简述
特殊功能 FB（节能优化 / 报警 / 防冻 / 多路选择 / 能耗测量等）：`FB_HVACFreezeProtectionHeater` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 6 个 VAR_INPUT 引脚与 8 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `bThermostat` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| `bFrostProtecRunning` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bHardwareReset` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bErrorGeneral` | `BOOL` | - | 错误指示位（General）。`bReset` 上升沿清错。 |
| `bAlarmReturnWaterTemp` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bAlarmThermostat` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `rReturnWaterAlarmLimit` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rReturnWaterNormalLimit` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `tDelayHardwareReset` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tMonitoringAfterReset` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACFreezeProtectionHeater.TcPOU`](../examples/P_Demo_FB_HVACFreezeProtectionHeater.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.7.12
