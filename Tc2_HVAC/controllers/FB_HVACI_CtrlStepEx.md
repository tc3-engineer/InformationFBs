...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685052171.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ⚠️ not-on-infosys |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACI_CtrlStepEx.TcPOU`](../examples/P_Demo_FB_HVACI_CtrlStepEx.TcPOU) |

---

## 1. 功能简述
通用控制器 FB（P / PI / PID / 步进控制器）：`FB_HVACI_CtrlStepEx` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 14 个 VAR_INPUT 引脚与 11 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `iStartStep` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
...
| `bStepUp` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bStepDown` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bDirection` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `iNumberOfStepInProfile` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `bBlock` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| `rI_Ctrl` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rE` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rW_High` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rW_Low` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `b_rW_High` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `b_rW_Low` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `udiSecRT_DelayHigh` | `UDINT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `udiSecRT_DelayLow` | `UDINT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACI_CtrlStepEx.TcPOU`](../examples/P_Demo_FB_HVACI_CtrlStepEx.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.4.3
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685052171.html
