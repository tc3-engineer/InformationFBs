...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/9682567307.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_GetDeviceIdentificationEx.TcPOU`](../examples/P_Demo_FB_GetDeviceIdentificationEx.TcPOU) |

---

## 1. 功能简述

FB_GetDeviceIdentificationEx 读取目标 TwinCAT 设备的综合身份信息：设备名 / TwinCAT 版本 / OS 版本 / Image 版本 等。返回的 `ST_DeviceIdentification` 结构覆盖设备硬件 + 软件栈的关键字段。
...
| `stDevIdent` | `ST_DeviceIdentificationEx` | 参数 `stDevIdent`（类型 `ST_DeviceIdentificationEx`）。⚠️ PDF 未详述含义，请按 §3 行为说明使用。 |
...
**输出字段**：通常包括 DeviceName、TwinCATVersion、Build、OSVersion、ImageVersion、ImageOSName 等；具体字段以 PDF / InfoSys 描述的 `ST_DeviceIdentification` 为准。
...
- TwinCAT 版本字段是字符串（如 `'3.1.4024.32'`），做版本对比时要拆字符串成 INT 而非字典序比较。（工程经验补充）

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_FB_GetDeviceIdentificationEx.TcPOU`](../examples/P_Demo_FB_GetDeviceIdentificationEx.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）。
>
...
- **场景**：部署前自动盘点所有 PLC 的 TwinCAT 版本，找出需要升级的机器。
- **价值**：替代手动 SSH / 远程登录看版本。
- **替代方案对比**：
  - 手动远端登录：批量痛。
  - **本 FB**：脚本化盘点。
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf) §3.26
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/9682567307.html
- **相关 FB**：`FB_GetSystemId`, `FB_GetRouterStatusInfo`
