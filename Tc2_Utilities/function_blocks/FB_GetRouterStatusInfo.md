...
| `bError` | `BOOL` | （详见 PDF） |
| `nErrID` | `UDINT` | （详见 PDF） |
| `info` | `ST_TcRouterStatusInfo` | （详见 PDF） |
...
- 见上方功能简述。
- 详细行为（时序、错误码、状态机）请对照 PDF 第 3.35 节。
...
- 描述句、时序行为、错误码表等细节请以 PDF 第 3.35 节为准（⚠️ 待人工细化）。

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_FB_GetRouterStatusInfo.xml`](../examples/P_Demo_FB_GetRouterStatusInfo.xml)
>
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_FB_GetRouterStatusInfo
VAR
    fbFB_GetRouterStatusInfo : FB_GetRouterStatusInfo;
    ...
    out_bError : BOOL;
    out_nErrID : UDINT;
    out_info : ST_TcRouterStatusInfo;
END_VAR
