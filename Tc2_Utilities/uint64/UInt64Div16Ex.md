...
**T_ULARGE_INTEGER ÷ WORD**：64-bit 除以 16-bit，返回商，余数通过 `remainder` 返回。
...
- **除零行为未定义**——调用方先检查 `divisor != 0`。
- 比 `UInt64Div64Ex` 快（特化 16-bit 除数）。

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_UInt64Div16Ex.xml`](../examples/P_Demo_UInt64Div16Ex.xml)
>
...
VAR
    ...
    bRun    : BOOL;
    a, q, r : T_ULARGE_INTEGER;
    b : WORD := 7;
END_VAR

a := ULARGE_INTEGER(0, 100);
IF bRun THEN
    rResult := UInt64Div16Ex(dividend := a, divisor := b, remainder := r);
    bRun := FALSE;
END_IF;
```
