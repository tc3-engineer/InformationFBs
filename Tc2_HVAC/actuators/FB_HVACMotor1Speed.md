...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4684921227.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACMotor1Speed.TcPOU`](../examples/P_Demo_FB_HVACMotor1Speed.TcPOU) |

---

## 1. 功能简述
执行器控制 FB（阀门 / 风阀 / 水泵 / 电机）：`FB_HVACMotor1Speed` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 12 个 VAR_INPUT 引脚与 12 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `bAuto` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| `bMotorProtec` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bFeedbContactor` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `bMotor` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `byState` | `BYTE` | - | 状态字（按位）：每一位对应一个联锁 / 状态信号，便于 HMI 统一回读多个布尔状态。 |
| `bStateAuto` | `BOOL` | - | 状态镜像位（Auto），用于把内部 / 输入状态回显给 HMI。 |
...
| `bErrorMotorprotec` | `BOOL` | - | 错误指示位（Motorprotec）。`bReset` 上升沿清错。 |
| `bErrorFeedbContactor` | `BOOL` | - | 错误指示位（FeedbContactor）。`bReset` 上升沿清错。 |
...
| `bErrorMotorprotec` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorFeedbContactor` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACMotor1Speed.TcPOU`](../examples/P_Demo_FB_HVACMotor1Speed.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.2.5
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4684921227.html
