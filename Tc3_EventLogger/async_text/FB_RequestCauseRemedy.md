...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc3_eventlogger/13723756427.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_RequestCauseRemedy.TcPOU`](../examples/P_Demo_FB_RequestCauseRemedy.TcPOU) |
...
每个事件可以在 EventClass 配置阶段绑定多组 cause/remedy（例如「为什么会触发该故障」+「怎么修」），本 FB 把它们异步拉取后通过 `Get(nIndex)` 按下标取出。返回的子 FB 是 `FB_TcCauseRemedy`。
...
| `fbCauseRemedy` | `REFERENCE TO FB_TcCauseRemedy` | ⚠️ 待人工确认（PDF/InfoSys Description 列为空或仅英文） |
...
> 配套可导入文件：[`examples/P_Demo_FB_RequestCauseRemedy.TcPOU`](../examples/P_Demo_FB_RequestCauseRemedy.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
HMI 操作员双击报警 → 后台异步取该报警的「原因/补救」列表 → 显示帮助文本
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc3_EventLogger_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_EventLogger_EN.pdf) §3.1.2
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc3_eventlogger/13723756427.html
