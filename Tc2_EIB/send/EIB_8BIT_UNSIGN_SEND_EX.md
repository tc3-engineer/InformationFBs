...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187774219.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_8BIT_UNSIGN_SEND_EX.TcPOU`](../examples/P_Demo_EIB_8BIT_UNSIGN_SEND_EX.TcPOU) |

---

## 1. 功能简述

**`EIB_8BIT_UNSIGN_SEND` 的扩展版本**：发送 DPT 5.005 / 5.010 (1-byte raw) telegram，支持 Manual / Polling / OnChange / OnChangePolling 4 种模式。

与非 _EX 版本相比，本 FB **支持周期定时重发**（Polling）和 **响应 EIB Read 请求**（`bEnableReadReq`）。实际工程里大多用 _EX 版本，因为业务上经常需要周期心跳或主动答查。
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `bStart` | `BOOL` | — | FB 触发输入。`iMode = 0` 时上升沿触发；`iMode = 1/2/3` 时电平 = 使能 |
| `iMode` | `INT` | — | 0 = Manual / 1 = Polling / 2 = OnChange / 3 = OnChangePolling，见 §3 |
| `CyclePolling` | `TIME` | `t#500ms` | Polling 模式下的发送周期。**下限 200 ms**（库强制） |
| `MinSendTime` | `TIME` | `t#1s` | OnChange 模式下的最短发送间隔。**下限 200 ms**（库强制） |
| `Group_Address` | `EIB_GROUP_ADDR` | — | **目标 EIB 组地址**（必须在 KL6301 过滤器内） |
| `byData` | `BYTE` | — | 原始 byte 数据，0x00..0xFF |
| `str_Rec` | `EIB_REC` | — | 收发胶水结构，必须传 `KL6301.str_Data_Rec` 同一个实例 |
| `bEnableReadReq` | `BOOL` | — | 使能响应 EIB Read_Group_Req（其它设备主动查值时本 FB 自动答） |
...
> 配套可导入文件：[`examples/P_Demo_EIB_8BIT_UNSIGN_SEND_EX.TcPOU`](../examples/P_Demo_EIB_8BIT_UNSIGN_SEND_EX.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）

> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_EIB_8BIT_UNSIGN_SEND_EX
VAR
    fb       : EIB_8BIT_UNSIGN_SEND_EX;
    stEibRec : EIB_REC;
    stGroup  : EIB_GROUP_ADDR := (MAIN := 1, SUB_MAIN := 2, NUMBER := 21);
    data     : BYTE := BYTE#0;
END_VAR
fb(bStart := TRUE, iMode := 3, CyclePolling := T#5S, MinSendTime := T#500MS,
   Group_Address := stGroup, byData := data, str_Rec := stEibRec,
   bEnableReadReq := TRUE);
```

## 7. 业务场景与实际价值

- **场景**：PLC 设备状态码（原始 byte）定时心跳 + 即时变化 + 响应主动查询
- **价值**：BMS 工程的实际首选方式
- **替代方案对比**：
  - `EIB_8BIT_UNSIGN_SEND`：只变化发，无周期
  - 本 _EX：BMS 工程**首选**

## 8. 参考资料

- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.18
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187774219.html
- **相关**：`EIB_8BIT_UNSIGN_SEND`（同库 §4.2.5.17，简化版）、对应接收 FB
