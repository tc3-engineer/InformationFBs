...
**USINT → LREAL（unsigned-safe）**：TC2 ARM 平台原生的 unsigned → LREAL 转换在最高位置 1 时可能错误转为负数。本函数显式保证按正数转换。
...
> 配套可导入文件：[`examples/P_Demo_USINT_TO_LREALEX.xml`](../examples/P_Demo_USINT_TO_LREALEX.xml)
>
