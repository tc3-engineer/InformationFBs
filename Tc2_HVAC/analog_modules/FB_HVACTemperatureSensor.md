...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4684942603.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACTemperatureSensor.TcPOU`](../examples/P_Demo_FB_HVACTemperatureSensor.TcPOU) |

---

## 1. 功能简述
模拟输入 / 输出 FB（量程换算、传感器处理、温度曲线）：`FB_HVACTemperatureSensor` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 8 个 VAR_INPUT 引脚与 9 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `bErrorShortCircuit` | `BOOL` | - | 错误指示位（ShortCircuit）。`bReset` 上升沿清错。 |
| `bErrorBrokenSensor` | `BOOL` | - | 错误指示位（BrokenSensor）。`bReset` 上升沿清错。 |
| `bErrorHighLimit` | `BOOL` | - | 错误指示位（HighLimit）。`bReset` 上升沿清错。 |
| `bErrorLowLimit` | `BOOL` | - | 错误指示位（LowLimit）。`bReset` 上升沿清错。 |
| `bErrorCycleTime` | `BOOL` | - | 错误指示位（CycleTime）。`bReset` 上升沿清错。 |
...
| `bErrorShortCircuit` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorBrokenSensor` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorHighLimit` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorLowLimit` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorCycleTime` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACTemperatureSensor.TcPOU`](../examples/P_Demo_FB_HVACTemperatureSensor.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.3.10
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4684942603.html
