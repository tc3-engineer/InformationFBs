...
| Status | `verified` |
| Example | [`examples/P_Demo_FB_JsonDomParser_IsString.TcPOU`](../examples/P_Demo_FB_JsonDomParser_IsString.TcPOU) |

---

## 1. 功能简述

判断 JSON 节点的值是否为 STRING 类型。返回 `TRUE` 表示是。
...
> 配套可导入文件：[`examples/P_Demo_FB_JsonDomParser_IsString.TcPOU`](../examples/P_Demo_FB_JsonDomParser_IsString.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **场景**：对解析后的不可信 JSON 数据做类型验证（如校验外部下发字段是否真是 String 类型）。
- **价值**：避免 Get* 在类型不匹配时返回未定义值导致后续计算异常。
- **替代方案对比**：用 try/catch 兜底 → IEC 61131-3 不直接支持异常；不检查直接 Get → 偶发崩溃难定位。

## 8. 参考资料

- **PDF**：[`TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf) §4.1.66
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc3_jsonxml/4219231115.html
- **相关 FB / FC**：`FB_JsonDomParser`, `GetString`
