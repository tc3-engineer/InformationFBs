...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/13612626699.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_Gaussian.TcPOU`](../examples/P_Demo_FB_FTR_Gaussian.TcPOU) |

---

## 1. 功能简述

实现一个高斯滤波器（Gaussian filter）。输出是最近 M 个输入值的**加权平均**，权重由高斯函数计算，标准差 σ 决定截止频率。由配置结构 `ST_FTR_Gaussian` 给定截止频率 `fCutoff`、采样率与（可选的）窗口长度 `nSamplesToFilter`。用于在最小相位失真（群延迟很小）的前提下平滑信号。

> 类别归属：高斯平滑滤波器。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_Gaussian` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
> **PDF 原文如实记录（含笔误）**：PDF §5.1.15.1 的 `Configure` 示例把入参类型印成了下面这样——这是从其它滤波器复制粘贴遗留的**错误**，正确类型应为 `ST_FTR_Gaussian`（与上面 FB 定义、InfoSys topic 页一致）。下面这段仅用于忠实呈现 PDF 文本，请勿照抄：

```iecst
METHOD Configure : BOOL
VAR_INPUT
    stConfig : ST_FTR_PT1;   // ← PDF 笔误，应为 ST_FTR_Gaussian
END_VAR
```

### 配置结构 `ST_FTR_Gaussian`（PDF §5.2.1.15）
...
> **关于 PDF 笔误**：PDF §5.1.15.1 的 Configure 示例把入参类型误写成 `stConfig : ST_FTR_PT1`（复制粘贴遗留），FB 定义中的 `stConfig` 与 InfoSys topic 页均为正确的 `ST_FTR_Gaussian`——以 InfoSys 为准。

## 3. 行为说明

**配置 → 调用 → 复位三段式**：本 FB 不在每个周期『自动跑』，而是由调用方在每个采样周期主动调用 `Call()` 方法推进一步。典型生命周期分三步：

1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_Gaussian(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
截止频率 `fCutoff` 由高斯标准差 σ 与采样率共同定义，使归一化幅频响应在截止频率处取 `1/√2 ≈ -3 dB`；`fCutoff` 必须大于 0 且小于 `fSamplingRate/2`。与移动平均（矩形窗、等权重）相比，高斯窗对中心点加权最大、向两侧指数衰减，因此频率响应更平滑、无旁瓣振铃，群延迟更小、对信号波形的形变更轻。窗口长度 `nSamplesToFilter` 可填 `0`，此时库按 `fCutoff` 自动计算合适的窗口长度。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_Gaussian(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_Gaussian.TcPOU`](../examples/P_Demo_FB_FTR_Gaussian.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：对一路传感器轮廓信号（如激光测厚的曲线）做平滑去噪，又要尽量不破坏曲线形状/不引入明显滞后，于是用高斯滤波而非移动平均。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_Gaussian，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：在群延迟（相位失真）最小的前提下平滑信号，频率响应无旁瓣振铃——比移动平均更『温柔』，更适合保形去噪。窗口长度可交给库按截止频率自动定。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_Gaussian;                  // 被演示的滤波器实例
    stParams      : ST_FTR_Gaussian := (fCutoff := 100, fSamplingRate := 1000, nSamplesToFilter := 0, nOversamples := 1, nChannels := 1);
...
- **场景**：对一路传感器轮廓信号（如激光测厚的曲线）做平滑去噪，又要尽量不破坏曲线形状/不引入明显滞后，于是用高斯滤波而非移动平均。
- **价值**：在群延迟（相位失真）最小的前提下平滑信号，频率响应无旁瓣振铃——比移动平均更『温柔』，更适合保形去噪。窗口长度可交给库按截止频率自动定。
- **替代方案对比**：`FB_FTR_MovAvg`（矩形窗）群延迟大、有旁瓣；需要保形、低相位失真的平滑用高斯滤波。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.15（功能块）、§5.2.1.15（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/13612626699.html
