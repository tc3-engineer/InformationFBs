...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/5855745803.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_PT2.TcPOU`](../examples/P_Demo_FB_FTR_PT2.TcPOU) |

---

## 1. 功能简述

实现一个二阶延迟环节（PT2 element），由两个串联一阶环节构成，拉普拉斯域传递函数 `G(s) = Kp / ((1 + T1·s)(1 + T2·s))`。由配置结构 `ST_FTR_PT2` 给定增益 `fKp`、两个时间常数 `fT1`/`fT2` 与采样率。

> 类别归属：二阶延迟环节（PT2）。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_PT2` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_PT2(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
比 PT1 多一个时间常数，滚降更陡（高频衰减约 -40 dB/decade），阶跃响应呈无超调的 S 形上升（两个实极点、非振荡）。两个时间常数可不同；总体响应快慢主要由较大的那个时间常数主导。若需要带阻尼振荡特性（复极点）请改用 `FB_FTR_PT2oscillation`。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_PT2(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_PT2.TcPOU`](../examples/P_Demo_FB_FTR_PT2.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：对一路液位信号做更强的平滑（比 PT1 滚降更陡），同时保证阶跃响应不出现超调，用 PT2（两实极点）取代单个 PT1。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_PT2，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：在不引入振荡的前提下提供比 PT1 更强的高频抑制；两个时间常数独立可调，便于匹配被控对象动态。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_PT2;                  // 被演示的滤波器实例
    stParams      : ST_FTR_PT2 := (fKp := 1, fT1 := 0.3, fT2 := 0.1, fSamplingRate := 1000, nOversamples := 1, nChannels := 1);
...
- **场景**：对一路液位信号做更强的平滑（比 PT1 滚降更陡），同时保证阶跃响应不出现超调，用 PT2（两实极点）取代单个 PT1。
- **价值**：在不引入振荡的前提下提供比 PT1 更强的高频抑制；两个时间常数独立可调，便于匹配被控对象动态。
- **替代方案对比**：串两个 `FB_FTR_PT1`：可行但要两个实例与中间变量；本 FB 一次配置即得二阶特性。需要振荡阻尼特性时用 `FB_FTR_PT2oscillation`。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.6（功能块）、§5.2.1.6（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/5855745803.html
