...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11315187467.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_Median.TcPOU`](../examples/P_Demo_FB_FTR_Median.TcPOU) |

---

## 1. 功能简述

实现一个中值滤波器（median filter）。输出是最近若干个输入值排序后处于中间位置的那个值，窗口长度由配置结构 `ST_FTR_Median` 的 `nSamplesToFilter` 给出。中值滤波对孤立尖峰（脉冲噪声 / 离群点）的抑制远好于移动平均。

> 类别归属：中值滤波器。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_Median` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
### 配置结构 `ST_FTR_Median`（PDF §5.2.1.13）
...
1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_Median(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
中值是把窗口内数据按大小排序后取中间值：一半数据比它小、一半比它大。与移动平均（线性滤波）不同，中值是非线性滤波——单个极端离群点不会被『平均』进结果，而是被直接剔除，因此能保边缘、抗脉冲噪声。窗口长度通常取奇数，越长抑制越强但响应越慢；对连续高频白噪声的平滑效果不如移动平均。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_Median(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_Median.TcPOU`](../examples/P_Demo_FB_FTR_Median.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：一路距离传感器偶尔蹦出单点野值（反光/抖动造成），用 5 点中值滤波把这些孤立尖峰直接剔除，又不像移动平均那样把真实跳变抹圆。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_Median，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：对脉冲型离群点抑制极强且能保留真实阶跃边缘——这是移动平均（会把尖峰平均进结果、把边缘抹圆）做不到的。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_Median;                  // 被演示的滤波器实例
    stParams      : ST_FTR_Median := (nSamplesToFilter := 5, nOversamples := 1, nChannels := 1);
...
- **场景**：一路距离传感器偶尔蹦出单点野值（反光/抖动造成），用 5 点中值滤波把这些孤立尖峰直接剔除，又不像移动平均那样把真实跳变抹圆。
- **价值**：对脉冲型离群点抑制极强且能保留真实阶跃边缘——这是移动平均（会把尖峰平均进结果、把边缘抹圆）做不到的。
- **替代方案对比**：`FB_FTR_MovAvg`（移动平均）对孤立尖峰抑制差、会糊掉边缘；脉冲噪声/野值场景用中值滤波。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.13（功能块）、§5.2.1.13（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11315187467.html
