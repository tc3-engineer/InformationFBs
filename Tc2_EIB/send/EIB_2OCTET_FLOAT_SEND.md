...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187754251.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_2OCTET_FLOAT_SEND.TcPOU`](../examples/P_Demo_EIB_2OCTET_FLOAT_SEND.TcPOU) |

---

## 1. 功能简述

**发送 DPT 9.xxx (2-byte float, 半精度) telegram**：把 IEC `REAL` 值经库编码成 EIB 标准格式，发送到指定 `Group_Address`。**只在 `rData` 值变化时下发**——周期重发要用 `EIB_2OCTET_FLOAT_SEND_EX`。
...
| `rData` | `REAL` | — | 数据值（IEC `REAL`），库自动转成 EIB 2 byte float 编码（DPT 9.xxx 半精度浮点） |
...
> 配套可导入文件：[`examples/P_Demo_EIB_2OCTET_FLOAT_SEND.TcPOU`](../examples/P_Demo_EIB_2OCTET_FLOAT_SEND.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
...
- **场景**：PLC 测量值（温度 / 湿度 / 风速）发送到 BMS
- **价值**：自动编码半精度浮点 + 变化触发；DPT 9.xxx 标准发送
- **替代方案对比**：
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.1
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187754251.html
- **相关**：`EIB_2OCTET_FLOAT_SEND_EX`（同库 §4.2.5.2）、`EIB_2OCTET_FLOAT_REC`（接收端）
