...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/9007203939877259.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACStartAirConditioning.TcPOU`](../examples/P_Demo_FB_HVACStartAirConditioning.TcPOU) |

---

## 1. 功能简述
特殊功能 FB（节能优化 / 报警 / 防冻 / 多路选择 / 能耗测量等）：`FB_HVACStartAirConditioning` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 15 个 VAR_INPUT 引脚与 12 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `iStartSeqWinter` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `iStartSeqSummer` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `bTimeScheduler` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bNightCooling` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bFreezeProtec` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bOverHeatingProtec` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `rOutsideTemp` | `REAL` | - | 室外温度（℃，工程量）。 |
| `rReturnWaterTemp` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rSupplyAirTemp` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rReturnAirTemp` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `bDampersFeedbOpen` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bEmergencyStop` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| `bPreRinse` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bDampersOpen` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bSupplyVentilation` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bExhaustVentilation` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bStartPIDControllers` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bEnableLimitCtrl` | `BOOL` | - | TRUE 启用 `rHighLimit` / `rLowLimit` 限值监测。 |
| `bErrorGeneral` | `BOOL` | - | 错误指示位（General）。`bReset` 上升沿清错。 |
| `bErrorNoHotWater` | `BOOL` | - | 错误指示位（NoHotWater）。`bReset` 上升沿清错。 |
| `bErrorDampers` | `BOOL` | - | 错误指示位（Dampers）。`bReset` 上升沿清错。 |
...
| `bErrorNoHotWater` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
| `bErrorDampers` | PDF 同名描述段的错误条件 | `bReset` 上升沿清错；查 §2 表内对应描述 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACStartAirConditioning.TcPOU`](../examples/P_Demo_FB_HVACStartAirConditioning.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.7.29
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/9007203939877259.html
