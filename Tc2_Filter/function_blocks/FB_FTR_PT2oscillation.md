...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11014291467.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_PT2oscillation.TcPOU`](../examples/P_Demo_FB_FTR_PT2oscillation.TcPOU) |

---

## 1. 功能简述

实现一个**可振荡**的二阶延迟环节（oscillating PT2 element），拉普拉斯域传递函数 `G(s) = Kp / (1 + 2·θ·T1·s + (T1·s)^2)`。由配置结构 `ST_FTR_PT2oscillation` 给定增益 `fKp`、时间常数 `fT1` 与阻尼系数 `fTheta`（θ）。与两实极点的 `FB_FTR_PT2` 不同，本环节可呈欠阻尼振荡。

> 类别归属：振荡型二阶延迟环节（PT2）。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_PT2oscillation` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
### 配置结构 `ST_FTR_PT2oscillation`（PDF §5.2.1.11）

滤波器的全部参数通过 `stConfig` 这一个入参传入。结构定义（逐字照搬 PDF）：

```iecst
TYPE ST_FTR_PT2oscillation :
STRUCT
    fKp                : LREAL;
    fT1                : LREAL;
    fTheta             : LREAL;
...
> **关于 PDF 笔误**：PDF §5.2.1.11 结构定义里把类型头误写成 `TYPE ST_FTR_PT2`，但成员清单含 `fTheta`、且 FB 定义为 `stConfig : ST_FTR_PT2oscillation`——以 InfoSys topic 页为准，正确结构名是 `ST_FTR_PT2oscillation`。

## 3. 行为说明

**配置 → 调用 → 复位三段式**：本 FB 不在每个周期『自动跑』，而是由调用方在每个采样周期主动调用 `Call()` 方法推进一步。典型生命周期分三步：

1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_PT2oscillation(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
阻尼系数 `fTheta`（θ）决定动态特性：`θ < 1` 时为欠阻尼，阶跃响应有超调和衰减振荡，θ 越小振荡越剧烈；`θ = 1` 为临界阻尼（无超调、最快无振荡）；`θ > 1` 退化为两实极点的过阻尼（类似普通 PT2）。常用来模拟弹簧-质量-阻尼这类二阶机械系统的动态响应。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_PT2oscillation(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_PT2oscillation.TcPOU`](../examples/P_Demo_FB_FTR_PT2oscillation.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：在仿真/测试台上用一个欠阻尼 PT2（θ = 0.3）模拟弹簧-质量-阻尼系统的阶跃响应，验证上层控制算法对带振荡被控对象的整定效果。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_PT2oscillation，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：提供带阻尼参数的二阶振荡环节，一个 `fTheta` 就能在过阻尼与欠阻尼之间连续调节，是普通两实极点 PT2 无法表达的振荡动态。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_PT2oscillation;                  // 被演示的滤波器实例
    stParams      : ST_FTR_PT2oscillation := (fKp := 1, fT1 := 0.05, fTheta := 0.3, fSamplingRate := 1000, nOversamples := 1, nChannels := 1);
...
- **场景**：在仿真/测试台上用一个欠阻尼 PT2（θ = 0.3）模拟弹簧-质量-阻尼系统的阶跃响应，验证上层控制算法对带振荡被控对象的整定效果。
- **价值**：提供带阻尼参数的二阶振荡环节，一个 `fTheta` 就能在过阻尼与欠阻尼之间连续调节，是普通两实极点 PT2 无法表达的振荡动态。
- **替代方案对比**：`FB_FTR_PT2`（两实极点）只能做非振荡过阻尼；需要欠阻尼/超调振荡时必须用本 FB。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.11（功能块）、§5.2.1.11（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11014291467.html
