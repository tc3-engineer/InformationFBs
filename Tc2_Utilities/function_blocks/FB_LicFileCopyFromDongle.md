...
| `nPortSrc` | `UINT` | （详见 PDF） |
| `sNetIdDest` | `T_AmsNetId` | （详见 PDF） |
| `sFileNameSrc` | `STRING` | （详见 PDF） |
| `sFilePathNameDest` | `T_MaxString` | （详见 PDF） |
...
- 见上方功能简述。
- 详细行为（时序、错误码、状态机）请对照 PDF 第 3.40 节。
...
- 描述句、时序行为、错误码表等细节请以 PDF 第 3.40 节为准（⚠️ 待人工细化）。

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_FB_LicFileCopyFromDongle.xml`](../examples/P_Demo_FB_LicFileCopyFromDongle.xml)
>
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_FB_LicFileCopyFromDongle
VAR
    fbFB_LicFileCopyFromDongle : FB_LicFileCopyFromDongle;
    arg_sNetIdSrc : T_AmsNetId;
    arg_nPortSrc : UINT;
    arg_sNetIdDest : T_AmsNetId;
    arg_sFileNameSrc : STRING;
    arg_sFilePathNameDest : T_MaxString;
    arg_pCopyBuff : PVOID;
    arg_cbCopyLen : UDINT;
    arg_bExecute : BOOL;
...
fbFB_LicFileCopyFromDongle(
    sNetIdSrc := arg_sNetIdSrc,
    nPortSrc := arg_nPortSrc,
    sNetIdDest := arg_sNetIdDest,
    sFileNameSrc := arg_sFileNameSrc,
    sFilePathNameDest := arg_sFilePathNameDest,
