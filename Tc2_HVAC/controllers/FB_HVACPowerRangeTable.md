...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685061259.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACPowerRangeTable.TcPOU`](../examples/P_Demo_FB_HVACPowerRangeTable.TcPOU) |

---

## 1. 功能简述
通用控制器 FB（P / PI / PID / 步进控制器）：`FB_HVACPowerRangeTable` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 11 个 VAR_INPUT 引脚与 15 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `iNumberOfProfiles` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `iProfile` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `iStep` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `eCtrlModeProfile` | `E_HVACCtrlMode` | - | 枚举 / 结构（参见 `E_HVACCtrlMode`）。 |
| `iManualProfile` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `eCtrlModeStep` | `E_HVACCtrlMode` | - | 枚举 / 结构（参见 `E_HVACCtrlMode`）。 |
| `iManualStep` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `iNumberOfAggregates` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `bEnablePowerRangeTable` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `iNumberOfStepInProfile` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `iCurrentProfile` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `iCurrentStep` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `stI_Ctrl` | `ST_HVACI_Ctrl` | - | 枚举 / 结构（参见 `ST_HVACI_Ctrl`）。 |
| `stAggregate1` | `ST_HVACAggregate` | - | 枚举 / 结构（参见 `ST_HVACAggregate`）。 |
| `stAggregate2` | `ST_HVACAggregate` | - | 枚举 / 结构（参见 `ST_HVACAggregate`）。 |
| `stAggregate3` | `ST_HVACAggregate` | - | 枚举 / 结构（参见 `ST_HVACAggregate`）。 |
| `stAggregate4` | `ST_HVACAggregate` | - | 枚举 / 结构（参见 `ST_HVACAggregate`）。 |
| `stAggregate5` | `ST_HVACAggregate` | - | 枚举 / 结构（参见 `ST_HVACAggregate`）。 |
| `stAggregate6` | `ST_HVACAggregate` | - | 枚举 / 结构（参见 `ST_HVACAggregate`）。 |
| `bError` | `BOOL` | - | 通用错误指示位。`bReset` 上升沿清错。 |
| `eErrorCode` | `E_HVACErrorCodes` | - | 错误码枚举（参见 `E_HVACErrorCodes`）。错误发生时填具体编号。 |
| `iErrorPosArrayProfile` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `iErrorPosArrayStep` | `INT` | - | 整型工程量（语义见 PDF 同名描述段）。 |

#### VAR_IN_OUT 引脚描述

| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `arrPowerRangeTable` | `ARRAY [1..g_iMaxNumberOfProfiles,0..g_iMaxNumberOfSteps] OF ST_HVACPowerRange` | - | 语义见 PDF 同名描述段。 |

## 3. 行为说明

`FB_HVACPowerRangeTable` 是 Tc2_HVAC 库 通用控制器 FB（P / PI / PID / 步进控制器） 子类中的功能块。按 §2 接口定义表列出的引脚顺序，每周期单次调用本 FB；输入信号通过 VAR_INPUT 引脚传入、输出结果通过 VAR_OUTPUT 引脚回读，状态 / 错误位与同库其他 FB 的命名约定保持一致。 上电后 FB 处于禁用态，所有输出回到安全值；`bEnable := TRUE` 触发 FB 进入正常工作。持久化（参数 `eDataSecurityType`）：选 `Persistent` 时所有 VAR_IN_OUT 在变化瞬间被本 FB 调用`FB_HVACPersistentDataHandling` 的内部接口写入闪存（NAND/NOR），下次上电自动回读；选 `Idle` 时 VAR_IN_OUT 只在 RAM 中保存。 错误处理：`bError*` 系列输出一旦置 TRUE 则锁存（不会自动复位），必须在故障原因消除后给 `bReset` 一次FALSE→TRUE 上升沿才能清错。`bInvalidParameter` 表示 VAR_IN_OUT 参数超出 PDF 列出的合法范围（量程 / 时间 / 长度），排错时优先检查上述参数。 每个 PLC 周期都应调用本 FB 一次（不要条件调用、不要在不同任务里调用同一实例）；FB 内部维护状态机 / 积分量 / 时间累积，跳过调用会让计数 / 时序不准。按Tc2_HVAC 全库统一约定，所有输出在 FB 被调用的同一周期内更新，调用方可立即读取输出。
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACPowerRangeTable.TcPOU`](../examples/P_Demo_FB_HVACPowerRangeTable.TcPOU)
>
...
- **场景**：HVAC 控制回路：温度 / 压力 / 湿度 / 流量等连续量的 PID / 步进控制，与执行器 FB 串联使用。
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.4.6
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685061259.html
