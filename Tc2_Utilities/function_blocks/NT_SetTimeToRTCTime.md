...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/35038603.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_NT_SetTimeToRTCTime.TcPOU`](../examples/P_Demo_NT_SetTimeToRTCTime.TcPOU) |

---

## 1. 功能简述

NT_SetTimeToRTCTime 把硬件 RTC（CMOS 时钟）的当前值写回 Windows 系统时间。用于硬件 RTC 是权威时间源、Windows 时间被某程序意外改变后的恢复操作。
...
- **硬件 RTC 精度有限**：CX 工控机的 CMOS 电池没电时硬件 RTC 不准；要先确认 RTC 健康再用。（工程经验补充）
- 调用频率不要高——这是恢复操作而不是周期同步。

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_NT_SetTimeToRTCTime.TcPOU`](../examples/P_Demo_NT_SetTimeToRTCTime.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）。
>
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf) §3.69
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/35038603.html
- **相关 FB**：`NT_GetTime`, `NT_SetLocalTime`
