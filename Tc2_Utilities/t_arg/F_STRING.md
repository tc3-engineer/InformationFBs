...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/35258379.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_F_STRING.TcPOU`](../examples/P_Demo_F_STRING.TcPOU) |

---

## 1. 功能简述

`F_STRING` 是 `T_Arg`（参数描述结构体）的**辅助打包函数**。给定一个 `T_MaxString` 类型的变量，返回一个 `T_Arg` 结构体，里面记录了该变量的**类型枚举**（`eType`，本函数固定为 `T_MaxString` 对应的常量）、**字节长度**（`cbLen`，本类型 = 256 字节）以及**指向原始数据的指针**（`pData = ADR(in)`）。
...
本函数对应 `T_MaxString` 类型（最长 T_MaxString（默认 STRING(255)））。
...
| 名称 | 类型 | 方向 | 说明（中文） |
|---|---|---|---|
| `in` | `T_MaxString` | `VAR_IN_OUT` | 待打包的原始变量。函数会取该变量的地址 `ADR(in)` 写入返回 `T_Arg.pData`，所以**必须**通过引用传递（VAR_IN_OUT），不能传字面量 |
...
```
F_STRING.eType  := eValueType_T_MaxString     /* 在 E_ArgType 枚举中对应 T_MaxString 的常量 */
F_STRING.cbLen  := SIZEOF(T_MaxString)        /* 本类型 = 256 字节 */
F_STRING.pData  := ADR(in)                  /* 指向原变量内存 */
```
...
```
sLogLine : T_MaxString;
argList : T_ArgList;
...
F_PutInArg(arg := F_STRING(sLogLine), nIdx := 0, putState := PUTARG_INIT, args := argList);
...
- **不能传字面量**。`F_STRING('Pump 1 alarm: low flow')` 之类的写法编译报错——VAR_IN_OUT 必须传可寻址左值。要传常量，先赋给一个临时变量再传。
- **生命期陷阱**。返回的 `T_Arg.pData` 是原变量地址。如果在一个函数里返回 `F_STRING(localVar)`，离开函数后 `localVar` 已被回收，下游再用就读到栈垃圾。安全做法：在 POU 实例的稳定变量（VAR）上调用，不要在 FUNCTION 的临时局部变量上调用。
...
> 配套可导入文件：[`examples/P_Demo_F_STRING.TcPOU`](../examples/P_Demo_F_STRING.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：在事件日志/格式化字符串中把一个 T_MaxString 业务量打包成 T_Arg，
...
// 验证：在线写 sLogLine 为示例值，bPackArg := TRUE，
//       观察 argDescriptor.cbLen 等于 SIZEOF(T_MaxString)、argDescriptor.pData 非 0。
PROGRAM P_Demo_F_STRING
VAR
    sLogLine                        : T_MaxString := 'Pump 1 alarm: low flow';  // 业务量
...
IF bPackArg THEN
    ...
    argDescriptor := F_STRING(sLogLine);
    bPackArg := FALSE;
END_IF;
```
...
- **场景**：把 `T_MaxString` 业务量（最长 T_MaxString（默认 STRING(255)））作为可变参数喂给 `Tc3_EventLogger`、`Tc2_System.FB_FormatString2`、`Tc2_System.ADSWRITE` 等下游消费者；或者用作 ADS Notification 自定义消息的字段打包。
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf) §4.10.17
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/35258379.html
