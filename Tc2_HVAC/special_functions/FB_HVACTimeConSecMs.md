...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685143947.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACTimeConSecMs.TcPOU`](../examples/P_Demo_FB_HVACTimeConSecMs.TcPOU) |

---

## 1. 功能简述
特殊功能 FB（节能优化 / 报警 / 防冻 / 多路选择 / 能耗测量等）：`FB_HVACTimeConSecMs` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 0 个 VAR_INPUT 引脚与 0 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `udiMs` | `UDINT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `udiSec` | `UDINT` | - | 整型工程量（语义见 PDF 同名描述段）。 |
| `tTime` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |

## 3. 行为说明

`FB_HVACTimeConSecMs` 是 Tc2_HVAC 库 特殊功能 FB（节能优化 / 报警 / 防冻 / 多路选择 / 能耗测量等） 子类中的功能块。按 §2 接口定义表列出的引脚顺序，每周期单次调用本 FB；输入信号通过 VAR_INPUT 引脚传入、输出结果通过 VAR_OUTPUT 引脚回读，状态 / 错误位与同库其他 FB 的命名约定保持一致。 每个 PLC 周期都应调用本 FB 一次（不要条件调用、不要在不同任务里调用同一实例）；FB 内部维护状态机 / 积分量 / 时间累积，跳过调用会让计数 / 时序不准。按Tc2_HVAC 全库统一约定，所有输出在 FB 被调用的同一周期内更新，调用方可立即读取输出。
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACTimeConSecMs.TcPOU`](../examples/P_Demo_FB_HVACTimeConSecMs.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.7.34
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685143947.html
