...
| Status | `verified` |
| Example | [`examples/P_Demo_FB_JsonDomParser_MemberEnd.TcPOU`](../examples/P_Demo_FB_JsonDomParser_MemberEnd.TcPOU) |

---

## 1. 功能简述

返回 JSON 对象节点的末成员的迭代器（end，循环终止条件）的成员迭代器 `SJsonMIterator`，配合 `NextMember()` 遍历对象的所有键值对。
...
> 配套可导入文件：[`examples/P_Demo_FB_JsonDomParser_MemberEnd.TcPOU`](../examples/P_Demo_FB_JsonDomParser_MemberEnd.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **PDF**：[`TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf) §4.1.71
