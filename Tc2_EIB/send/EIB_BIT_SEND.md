...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187778827.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_BIT_SEND.TcPOU`](../examples/P_Demo_EIB_BIT_SEND.TcPOU) |

---

## 1. 功能简述

**发送 DPT 1.xxx (1-bit) telegram**：把 IEC `BOOL` 值经库编码成 EIB 标准格式，发送到指定 `Group_Address`。**只在 `bData` 值变化时下发**——周期重发要用 `EIB_BIT_SEND_EX`。

**最小发送间隔由库内部固化为 200 ms**：在该间隔内的连续变化会被合并到最后一次值。如果值在间隔内变化又回到旧值，**不会**发新帧（PDF §4.2.5.x 明确）。
...
**触发方式**：每周期调用本 FB；**只要数据值（bData）发生变化**，库就把新值编码成 EIB telegram 并下发到 EIB 总线。**初次调用**也会发一次（首次值通过）。

**最小发送间隔**：本 FB 不带 `MinSendTime` 引脚，但库内部固化了 200 ms（PDF §4.2.5.x 描述）。在间隔内连续变化会被合并——最后一次值在间隔到期后被发出（PDF "No new EIB telegram is sent if the value changes within the min. send time but falls back to the old, already sent value within the min. send time" 描述：变化又回到旧值则不发新帧）。
...
> 配套可导入文件：[`examples/P_Demo_EIB_BIT_SEND.TcPOU`](../examples/P_Demo_EIB_BIT_SEND.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）

> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_EIB_BIT_SEND
VAR
    fb       : EIB_BIT_SEND;
    stEibRec : EIB_REC;
    ...
    data     : BOOL := FALSE;
END_VAR
fb(Group_Address := stGroup, bData := data, str_Rec := stEibRec);
```

## 7. 业务场景与实际价值

- **场景**：PLC 开关命令发送到 BMS
- **价值**：DPT DPT 1.xxx 标准发送方式
- **替代方案对比**：
  - `EIB_BIT_SEND_EX`：周期发 + read 响应
  - `EIB_ALL_DATA_TYPES_SEND`：raw byte 模式
  - 本 FB：简单变化发送的标准选择

## 8. 参考资料

- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.22
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187778827.html
- **相关**：`EIB_BIT_SEND_EX`（同库 §4.2.5.23）、对应接收 FB
