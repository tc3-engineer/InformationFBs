...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192776843.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBUdpWriteSingleReg.TcPOU`](../examples/P_Demo_FB_MBUdpWriteSingleReg.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 6（Write Single Register）的客户端功能块：向远端 Modbus 设备写单个输出寄存器，采用 16 位访问。`nValue` 为要写入的数据字。`bExecute` 上升沿触发一次写入。 本块是 UDP 变体：底层用无连接的 Modbus/UDP 报文传输，接口与同名 TCP 版 `FB_MBWriteSingleReg` 完全一致，区别仅在传输层（不建立 TCP 连接，不保证可靠交付）。
...
**触发与状态机**：`bExecute` 上升沿触发一次写单寄存器操作。FB 通过 ADS 异步把请求转交给 TwinCAT Modbus TCP 服务组件，由它向 `sIPAddr` 指定的远端 Modbus 设备发出 UDP（无连接数据报）请求。请求发出后 `bBUSY := TRUE`；收到对端响应或超时后，`bBUSY := FALSE`，并据结果置 `bError` 与 `nErrId`。`bExecute` 在 `bBUSY` 期间保持高电平不会重复触发，必须等一次执行结束、再来一个上升沿才会重发。

**角色定位**：本库的 FB_MB* 系列让 PLC 充当 Modbus **主站（master/client）**，主动去读写远端设备的单个保持寄存器。它与“TwinCAT 自身作为 Modbus 从站被外部 SCADA 访问”是两回事——后者由 TF6250 服务端 + 配置器 + `mb_Input_Coils` 等 GVL 数组 + ADS 映射完成，不经过本 FB。
...
> 配套可导入文件：[`examples/P_Demo_FB_MBUdpWriteSingleReg.TcPOU`](../examples/P_Demo_FB_MBUdpWriteSingleReg.TcPOU)
>
...
- **场景**：向变频器/温控表写单个设定值，例如改写一台变频器的目标频率寄存器，或给温控表下发新的设定温度。 选用 UDP 版多见于同一控制柜/同网段、对实时性敏感且能容忍偶发丢包的场景，省去 TCP 建链开销。
- **价值**：单点写寄存器用功能码 6；本 FB 一次调用即可把一个 `WORD` 写到指定寄存器，省去报文构造。
- **替代方案对比**：替代是写多寄存器（功能码 16）写 1 个字——可行但更重；或裸 socket。单点设定本 FB 最简。 与 TCP 版 `FB_MBWriteSingleReg` 的取舍：需要可靠交付选 TCP，需要低延迟且可容忍丢包选本 UDP 版。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.11.6
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192776843.html
- **相关**：`FB_MBWriteSingleReg`（TCP 版）、`FB_MBWriteRegs`（批量写寄存器）、`stLibVersion_Tc2_ModbusSrv`（库版本）
