...
| Type | `FUNCTION_BLOCK` |
| Category | `Object · Schedule Binary` |
...
| Example | [`examples/P_Demo_FB_BACnet_SchedB.TcPOU`](../examples/P_Demo_FB_BACnet_SchedB.TcPOU) |

---

## 1. 功能简述

代表 BACnet 标准里的「Schedule Binary」对象类型,周程序时间表 — 按 Monday..Sunday 每天定义一组「时刻 → 值」对(类型 `BOOL`),stack 每分钟检查当前时刻,把对应值写到所有 `aObjectPropertyReferences` 列出的目标对象属性。Schedule 可以引用一个或多个 Calendar 做例外日(节假日不同时间表),也可直接在 `aException` 内联例外。本变体处理 BOOL 值(灯光、风机、阀门开关等)。 PDF §9.11 给完整示例。
...
| 当前值 | `bPresVal` / `fPresVal` / `nPresVal`(按变体取一) | `BOOL` | Present_Value(当前时段对应的 BOOL 值) |
...
> 配套可导入文件:[`examples/P_Demo_FB_BACnet_SchedB.TcPOU`](../examples/P_Demo_FB_BACnet_SchedB.TcPOU)

```iecst
PROGRAM P_Demo_FB_BACnet_SchedB
VAR
    fbSched : FB_BACnet_SchedB := (
        sObjectName := 'BinarySchedule',
...
- **PDF**:[TF8020_TC3_BACnet_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8020_TC3_BACnet_EN.pdf) §6.1.1(SchedA/B/M = Schedule Binary)、§9.11(Calendar + Schedule 完整示例)
...
- **相关 FB**:`FB_BACnet_SchedA` / `FB_BACnet_SchedM`(REAL / 多态变体)、`FB_BACnet_Cal`(节假日表)、`FB_BACnetRM_SchedB`(远端 Schedule 引用)
