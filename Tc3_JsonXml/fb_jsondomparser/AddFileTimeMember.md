...
| Status | `verified` |
| Example | [`examples/P_Demo_FB_JsonDomParser_AddFileTimeMember.TcPOU`](../examples/P_Demo_FB_JsonDomParser_AddFileTimeMember.TcPOU) |

---

## 1. 功能简述

在 JSON 对象节点下增加一个键值对成员，值类型为 FileTime。通过键名（`key` 参数）和具体值（`value` 参数）写入 JSON DOM 文档；返回新增成员的 `SJsonValue` 句柄。
...
> 配套可导入文件：[`examples/P_Demo_FB_JsonDomParser_AddFileTimeMember.TcPOU`](../examples/P_Demo_FB_JsonDomParser_AddFileTimeMember.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **场景**：组装 JSON 报文时往对象里加 FileTime 类型的字段（如批次号、温度采样值、报警码等）。
- **价值**：单次调用即写键值对，无需自己组字符串避免转义问题。
- **替代方案对比**：手写 STRING 拼 `'"key":' + value` → 大量转义和分隔符出错；用 SAX writer → 适合流式构造但随机插入字段不便。

## 8. 参考资料

- **PDF**：[`TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf) §4.1.8
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc3_jsonxml/4219231115.html
- **相关 FB / FC**：`FB_JsonDomParser`, `SetFileTime`, `FindMember`
