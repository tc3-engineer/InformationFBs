...
| Status | `verified` |
| Example | [`examples/P_Demo_FB_JsonReadWriteDataType_CopySymbolNameByAddress.TcPOU`](../examples/P_Demo_FB_JsonReadWriteDataType_CopySymbolNameByAddress.TcPOU) |

---

## 1. 功能简述

`FB_JsonReadWriteDataType.CopySymbolNameByAddress()` 是 PLC 符号-JSON 桥接 提供的一个工具方法，在解析/构造 JSON 或 XML 文档的特定步骤中使用。具体参数语义见下文 §2 接口定义表格与 §3 行为说明；参数语义按命名约定推导自该方法在 `FB_JsonReadWriteDataType` 内的位置。
...
| 名称 | 类型 | 说明 |
|---|---|---|
| `sName` | `STRING` | JSON 键名。 |


## 3. 行为说明

`FB_JsonReadWriteDataType.CopySymbolNameByAddress()` 是 PLC 符号-JSON 桥接 提供的一个工具方法，在解析/构造 JSON 或 XML 文档的特定步骤中使用。具体参数语义见下文 §2 接口定义表格与 §3 行为说明；参数语义按命名约定推导自该方法在 `FB_JsonReadWriteDataType` 内的位置。调用者需注意：本方法的调用语义与 `FB_JsonReadWriteDataType` 的整体行为一致——先确保父对象已正确初始化（FB_JsonReadWriteDataType 的 `initStatus` = `S_OK`、必要时已 ParseDocument 或 NewDocument），再调用本方法。返回值/输出参数需在调用方业务代码中显式检查；如出现非预期返回，可调 `ExceptionRaised()`（DOM 解析器）或检查 `hrErrorCode`（IO/异步方法）定位问题。
...
> 配套可导入文件：[`examples/P_Demo_FB_JsonReadWriteDataType_CopySymbolNameByAddress.TcPOU`](../examples/P_Demo_FB_JsonReadWriteDataType_CopySymbolNameByAddress.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **PDF**：[`TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf) §4.6.6
