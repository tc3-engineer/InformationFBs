...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187777291.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_BIT_CONTROL_SEND.TcPOU`](../examples/P_Demo_EIB_BIT_CONTROL_SEND.TcPOU) |

---

## 1. 功能简述

**发送 2-bit Controlled telegram**（DPT 2.xxx：带优先级控制的 1-bit 开关）。`bControl` 或 `bValue` 任一变化即触发；最小间隔 200 ms。对应「应急覆盖普通开关」语义。
...
| `bControl` | `BOOL` | — | DPT 2.xxx 控制位（强制 / 优先级位） |
| `bValue` | `BOOL` | — | DPT 2.xxx 数据位（实际开关值） |
...
**触发方式**：只要 `bControl` 或 `bValue` 任一值发生变化就立即编码并发送一帧 EIB telegram。PDF §4.2.5.20 明确：只在两个布尔位中至少一个变化时才下发，**两个都没变就不发**。

**最小发送间隔 200 ms**：200 ms 内的连续变化合并到最后一次值；若值在间隔内变化又回到旧值，则不发送新帧。这是 EIB 库统一约定。

**典型场景说明**：应急按钮按下时 PLC 推 `(bControl = TRUE, bValue = FALSE)` 强制关灯；应急解除时再推 `(bControl = TRUE, bValue = TRUE)` 或 `(bControl = FALSE, bValue = TRUE)`。DPT 2.xxx 接收设备会先看 `bControl` 决定是否覆盖普通指令——本 FB 只负责把组合编码下发，覆盖决策在接收端做。

**与 KL6301 依赖**：必须 `KL6301.bReady = TRUE`；同 PLC 任务；`str_Rec` 传同一实例；`Group_Address` 必须在过滤器内，否则发不出去。
...
> 配套可导入文件：[`examples/P_Demo_EIB_BIT_CONTROL_SEND.TcPOU`](../examples/P_Demo_EIB_BIT_CONTROL_SEND.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.20
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187777291.html
- **相关**：`EIB_BIT_CONTROL_SEND_EX`（§4.2.5.21）、`EIB_BIT_CONTROL_REC`
