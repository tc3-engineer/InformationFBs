...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf5100_tc3_nc_i/3283175691.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_CfgReconfigGroup.TcPOU`](../examples/P_Demo_CfgReconfigGroup.TcPOU) |

---

## 1. 功能简述

`CfgReconfigGroup` 反向操作——把 3D / FIFO 组撤销，把组内的所有轴释放回各自的 PTP 单轴组。常在加工结束后、需要把同一根伺服当成普通 PTP 轴做单轴定位时使用。
...
`CfgReconfigGroup` 走 ADS 同步调用流程：在 `bExecute` 检测到上升沿时把命令打包发给 NCI、进入 `bBusy = TRUE` 状态等回包；NC 端接受后 `bBusy = FALSE`，若执行失败则 `bErr = TRUE` 并把错误号写入 `nErrId`。注意 PDF 原文明确指出 `bBusy` 监视的是 NC 端『接受』命令的时间，不是命令真正执行完成的时间——也就是说 `bBusy = FALSE` 不等于动作做完，只是 ADS 调用收到了应答。
...
> 配套可导入文件：[`examples/P_Demo_CfgReconfigGroup.TcPOU`](../examples/P_Demo_CfgReconfigGroup.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **PDF**：[TF5100_TC3_NC_I_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF5100_TC3_NC_I_EN.pdf) §7.1.1.4
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf5100_tc3_nc_i/3283175691.html
- **相关 FB / FC**：`CfgBuild3DGroup`（反操作）、`CfgReconfigAxis`（单轴版）
