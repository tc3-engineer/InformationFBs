...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/35023115.html |
| Verified | 2026-05-11 ✅ |
| InfoSys-checked | ✅ 2026-05-11 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_ScopeServerControl.TcPOU`](../examples/P_Demo_FB_ScopeServerControl.TcPOU) |
...
| `eReqState` | `E_ScopeServerState` | `SCOPE_SERVER_IDLE` | 参数 `eReqState`（类型 `E_ScopeServerState`）。⚠️ PDF 未详述含义，请按 §3 行为说明使用。 |
| `sConfigFile` | `STRING` | - | 字符串输入：`sConfigFile`。 |
| `sSaveFile` | `STRING` | - | 字符串输入：`sSaveFile`。 |
| `tTimeout` | `TIME` | `DEFAULT_ADS_TIMEOUT` | ADS 调用超时时长。默认 `DEFAULT_ADS_TIMEOUT`（约 5 秒）。 |
| `bExternalTriggerEvent` | `BOOL` | `FALSE` | 输入布尔标志：`bExternalTriggerEvent`。具体语义见 §3 行为说明。 |
...
| `bDone` | `BOOL` | TRUE 表示请求成功完成一次；下次 `bExecute` 上升沿前保持 TRUE。 |
...
- **Scope Server 必须运行**——没装 / 没启动会立刻返回错。
- **Scope 配置文件路径要预先就绪**——动态指向不存在的配置会失败。（工程经验补充）
- **录制时间影响磁盘占用**——长时间录制会写大文件，要预估磁盘。（工程经验补充）

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_FB_ScopeServerControl.TcPOU`](../examples/P_Demo_FB_ScopeServerControl.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）。
>
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_Utilities_EN.pdf) §3.56
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_utilities/35023115.html
