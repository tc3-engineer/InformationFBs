...
| Type | `FUNCTION_BLOCK` |
| Category | `Object · Schedule Analog` |
...
| Example | [`examples/P_Demo_FB_BACnet_SchedA.TcPOU`](../examples/P_Demo_FB_BACnet_SchedA.TcPOU) |

---

## 1. 功能简述

代表 BACnet 标准里的「Schedule Analog」对象类型,周程序时间表 — 按 Monday..Sunday 每天定义一组「时刻 → 值」对(类型 `REAL`),stack 每分钟检查当前时刻,把对应值写到所有 `aObjectPropertyReferences` 列出的目标对象属性。Schedule 可以引用一个或多个 Calendar 做例外日(节假日不同时间表),也可直接在 `aException` 内联例外。本变体处理 REAL 值(温度设定点、阀位、转速等)。 PDF §9.11 给完整示例。
...
| 当前值 | `bPresVal` / `fPresVal` / `nPresVal`(按变体取一) | `REAL` | Present_Value(当前时段对应的 REAL 值) |
...
> 配套可导入文件:[`examples/P_Demo_FB_BACnet_SchedA.TcPOU`](../examples/P_Demo_FB_BACnet_SchedA.TcPOU)

```iecst
PROGRAM P_Demo_FB_BACnet_SchedA
VAR
    fbSched : FB_BACnet_SchedA := (
        sObjectName := 'AnalogSchedule',
...
- **PDF**:[TF8020_TC3_BACnet_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8020_TC3_BACnet_EN.pdf) §6.1.1(SchedA/B/M = Schedule Analog)、§9.11(Calendar + Schedule 完整示例)
...
- **相关 FB**:`FB_BACnet_SchedB` / `FB_BACnet_SchedM`(布尔 / 多态变体)、`FB_BACnet_Cal`(节假日表)、`FB_BACnetRM_SchedA`(远端 Schedule 引用)
