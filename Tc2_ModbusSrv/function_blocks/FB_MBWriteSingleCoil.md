...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192758539.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBWriteSingleCoil.TcPOU`](../examples/P_Demo_FB_MBWriteSingleCoil.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 5（Write Single Coil）的客户端功能块：向远端 Modbus 设备写单个数字量输出（线圈），采用位访问。`nValue` 取 `16#FF00` 置 ON、`16#0000` 置 OFF。`bExecute` 上升沿触发一次写入。
...
**触发与状态机**：`bExecute` 上升沿触发一次写单线圈操作。FB 通过 ADS 异步把请求转交给 TwinCAT Modbus TCP 服务组件，由它向 `sIPAddr` 指定的远端 Modbus 设备发出 TCP（端口 502，面向连接）请求。请求发出后 `bBUSY := TRUE`；收到对端响应或超时后，`bBUSY := FALSE`，并据结果置 `bError` 与 `nErrId`。`bExecute` 在 `bBUSY` 期间保持高电平不会重复触发，必须等一次执行结束、再来一个上升沿才会重发。
...
> 配套可导入文件：[`examples/P_Demo_FB_MBWriteSingleCoil.TcPOU`](../examples/P_Demo_FB_MBWriteSingleCoil.TcPOU)
>
...
- **场景**：远程控制单个继电器/电磁阀，例如令远端 IO 模块的某路输出闭合启动一台风机，或复位一个远程指示灯。
- **价值**：单点写线圈用功能码 5 最轻量；本 FB 一次调用、用 `16#FF00`/`16#0000` 明确 ON/OFF，无须拼包。
- **替代方案对比**：替代是写多线圈（功能码 15）写 1 位——可行但更重；或裸 socket 自实现。单点开关本 FB 最简。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.5
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192758539.html
- **相关**：`FB_MBUdpWriteSingleCoil`（UDP 版）、`FB_MBWriteCoils`（批量写线圈）、`stLibVersion_Tc2_ModbusSrv`（库版本）
