...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/5855741963.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_MovAvg.TcPOU`](../examples/P_Demo_FB_FTR_MovAvg.TcPOU) |

---

## 1. 功能简述

实现一个移动平均滤波器（moving average filter）。输出是最近 M 个输入值 `x[n-k]`（k = 0 … M−1）的算术平均，M 由配置结构 `ST_FTR_MovAvg` 的 `nSamplesToFilter` 给出（即窗口长度）。是最简单、最直观的信号平滑手段。

> 类别归属：移动平均滤波器。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_MovAvg` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
### 配置结构 `ST_FTR_MovAvg`（PDF §5.2.1.4）
...
1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_MovAvg(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
窗口越长平滑越强、但滞后（群延迟）越大、对阶跃的响应越慢。移动平均本质是一个 FIR 滤波器（无反馈，天然稳定），对周期性噪声有较好抑制：当窗口长度等于噪声周期的整数倍时该频率被完全滤除。刚启动或刚复位时窗口未填满，前几个输出会偏离稳态，可用初值预置（`pInitialValues`）缩短建立时间。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_MovAvg(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_MovAvg.TcPOU`](../examples/P_Demo_FB_FTR_MovAvg.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：一路称重传感器读数抖动明显，用 8 点移动平均把毛刺抹平后再送给上位机显示，让操作员看到的重量稳定、不跳字。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_MovAvg，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：一行配置即得平滑信号，无需自己维护环形缓冲与求和；FIR 结构天然稳定，不会发散。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_MovAvg;                  // 被演示的滤波器实例
    stParams      : ST_FTR_MovAvg := (nSamplesToFilter := 8, nOversamples := 1, nChannels := 1);
...
- **场景**：一路称重传感器读数抖动明显，用 8 点移动平均把毛刺抹平后再送给上位机显示，让操作员看到的重量稳定、不跳字。
- **价值**：一行配置即得平滑信号，无需自己维护环形缓冲与求和；FIR 结构天然稳定，不会发散。
- **替代方案对比**：自己开 `ARRAY` + 指针手写滑动求和：要处理回绕与除法，易出错；需要更可控的频率特性时用 `FB_FTR_IIRSpec`（低通）或 `FB_FTR_Gaussian`（更小群延迟）。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.4（功能块）、§5.2.1.4（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/5855741963.html
