...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11011665291.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_LeadLag.TcPOU`](../examples/P_Demo_FB_FTR_LeadLag.TcPOU) |

---

## 1. 功能简述

实现一个一阶相位校正环节（Lead-Lag element），拉普拉斯域传递函数 `G(s) = (1 + T1·s) / (1 + T2·s)`。由配置结构 `ST_FTR_LeadLag` 给定两个时间常数 `fT1`（分子/超前）、`fT2`（分母/滞后）与采样率。控制工程中用于补偿被控对象的相位、改善闭环稳定裕度。

> 类别归属：一阶超前/滞后（相位校正）环节。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_LeadLag` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
### 配置结构 `ST_FTR_LeadLag`（PDF §5.2.1.10）
...
> **关于 PDF 笔误**：PDF §5.1.10.1 的 Configure 示例与 §5.2.1.10 结构定义里把类型头误写成 `TYPE ST_FTR_PT2`，但成员清单（`fT1`/`fT2`/`fSamplingRate`/…）与 FB 定义中的 `stConfig : ST_FTR_LeadLag` 一致——以 InfoSys topic 页与 FB 定义为准，正确结构名是 `ST_FTR_LeadLag`。

## 3. 行为说明

**配置 → 调用 → 复位三段式**：本 FB 不在每个周期『自动跑』，而是由调用方在每个采样周期主动调用 `Call()` 方法推进一步。典型生命周期分三步：

1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_LeadLag(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
当 `fT1 > fT2` 时为超前（lead）环节，在中频段提供正相位，常用来增加相位裕度、加快响应；当 `fT1 < fT2` 时为滞后（lag）环节，在低频段提供增益、压低高频；`fT1 = fT2` 时退化为直通（增益 1）。与纯低通不同，Lead-Lag 的稳态增益为 1（直流不衰减），主要改变中高频的幅相特性。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_LeadLag(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_LeadLag.TcPOU`](../examples/P_Demo_FB_FTR_LeadLag.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：某速度闭环相位裕度不足、临界振荡，在前向通道串入一个超前环节（`fT1 > fT2`）在穿越频率处补回相位，把系统拉回稳定。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_LeadLag，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：提供经典的超前/滞后相位补偿，稳态增益保持 1（不影响直流跟踪精度），只调中高频幅相，是回路整定的常用工具。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_LeadLag;                  // 被演示的滤波器实例
    stParams      : ST_FTR_LeadLag := (fT1 := 0.01, fT2 := 0.002, fSamplingRate := 1000, nOversamples := 1, nChannels := 1);
...
- **场景**：某速度闭环相位裕度不足、临界振荡，在前向通道串入一个超前环节（`fT1 > fT2`）在穿越频率处补回相位，把系统拉回稳定。
- **价值**：提供经典的超前/滞后相位补偿，稳态增益保持 1（不影响直流跟踪精度），只调中高频幅相，是回路整定的常用工具。
- **替代方案对比**：自己离散化 `(1+T1 s)/(1+T2 s)`：要手推 z 域系数且易错；本 FB 内部用双线性变换自动离散化。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.10（功能块）、§5.2.1.10（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/11011665291.html
