...
| Type | `FUNCTION_BLOCK` |
| Category | `Primitive Value · Character String Value` |
...
| Example | [`examples/P_Demo_FB_BACnet_String.TcPOU`](../examples/P_Demo_FB_BACnet_String.TcPOU) |

---

## 1. 功能简述

代表 BACnet 标准里的「Character String Value」对象类型,Primitive Value 系列中的一个 — 用于把单个简单数据类型(整数 / 字符串 / 日期 / 时刻)直接暴露成可被 BMS 读写的 BACnet 对象,无需借助 AV / MV / String 之外的复杂对象类型。Primitive Value 对象本身没有命令优先级、没有 Status_Flags / Reliability,语义就是数据容器。本变体处理 BACnet 标准的 Character String(UTF-8 字符串)。PDF §6.1.2 把 10 种 Primitive Value 对象统一表述,§9.14 给完整声明 + 调用示例。
...
| 值 | `sValue` | `STRING` | Present_Value 容器(PLC 与 BMS 双向读写) |
...
> 配套可导入文件:[`examples/P_Demo_FB_BACnet_String.TcPOU`](../examples/P_Demo_FB_BACnet_String.TcPOU)

```iecst
PROGRAM P_Demo_FB_BACnet_String
VAR
    fbVal : FB_BACnet_String;
    val : STRING := 'TwinCAT BACnet';
END_VAR
fbVal.sValue := val;
fbVal();
```
...
- **场景**:把 PLC 端的最后一条错误消息、当前操作员姓名、正在运行的批次号等字符串暴露给 BMS。
