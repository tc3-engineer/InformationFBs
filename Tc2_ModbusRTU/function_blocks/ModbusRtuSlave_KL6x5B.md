...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6255_tc3_modbus_rtu/186540811.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_ModbusRtuSlave_KL6x5B.TcPOU`](../examples/P_Demo_ModbusRtuSlave_KL6x5B.TcPOU) |

---

## 1. 功能简述

通过串行总线端子 KL6001、KL6011 或 KL6021 通讯的 Modbus RTU 从站功能块（Modbus slave）。功能与 `ModbusRtuSlave_PcCOM` 完全相同，区别仅在底层硬件接口走 KL6x 总线端子而非 PC 的 COM 口。这三款端子数据过程映像较小（区别于 22 字节映像的 KL6031/KL6041，后者用 `ModbusRtuSlave_KL6x22B`）。

本功能块在收到主站报文前一直**被动等待**，不主动发起通讯，每个 PLC 周期调用一次即可，无需 `Execute`。工程把三块 Modbus 数据区声明成 PLC 数组并把地址与大小传入，主站即可按 Modbus 标准地址映射读写。与端子的链接所需数据结构内置在 FB 中，分配方式与 TF6340 TwinCAT 3 Serial Communication 文档「Serial bus terminal」章节一致。
...
举例：输入区 `Inputs[0]` 对应 Modbus 报文地址 `16#0`（设备地址 30001）；输出区 `Outputs[0]` 对应报文地址 `16#800`（40801）；存储区 `Memory[0]` 对应报文地址 `16#4000`（44001）。**主站发来的地址必须带这些偏移**，否则落到错误数据区或报地址非法。输入/输出区可用 `AT %I*` / `AT %Q*` 直接映射物理 I/O，也可声明为纯数据区。

**时序语义**：收到合法报文后在帧间隔内处理并应答；处理在本 FB 被调用的 PLC 周期推进。这几款端子数据吞吐相对小，主站连续读大批寄存器时应在主站侧适当加大 `Timeout`。通讯正常时 `ErrorId = MODBUSERROR_NO_ERROR`。
...
- **端子组态**：KL6001/6011/6021 串口参数用 KS2000 或 `Tc2_SerialCom` 的 `KL6configuration` 块设（配置不需 license，通讯需 TF6340），波特率/校验位与主站一致。
- **端子选型**：3 字节过程映像端子（KL6001/6011/6021）用本 FB；22 字节映像（KL6031/6041）用 `ModbusRtuSlave_KL6x22B`，选错型号过程数据链接对不上。（工程经验补充）

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_ModbusRtuSlave_KL6x5B.TcPOU`](../examples/P_Demo_ModbusRtuSlave_KL6x5B.TcPOU)
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK

```iecst
// 场景：TwinCAT 经 KL6011 端子作 Modbus RTU 从站，暴露 3 块数据区给上位主站。
PROGRAM P_Demo_ModbusRtuSlave_KL6x5B
VAR
    fbModbusSlave : ModbusRtuSlave_KL6x5B;
...
- **场景**：用 KL6001/KL6011/KL6021 串口端子把 TwinCAT 控制器接入 RS232/RS485 现场总线作 Modbus RTU 从站，被上位主站读写。这些端子相比 KL6031/6041 更经济，适合数据量不大的点对点串口从站。
- **价值**：与 `ModbusRtuSlave_PcCOM` 同样封装整套从站协议；硬件后端换成 KL6x 低速串口端子，业务代码无需改动。
- **替代方案对比**：
  - `ModbusRtuSlave_KL6x22B`：22 字节过程映像端子（KL6031/6041）。
  - `ModbusRtuSlave_PcCOM`：走 PC COM 口。
  - `ModbusRtuSlave_Generic`：硬件无关、可走虚拟 COM 口，但需 `Tc2_SerialCom` 配合且更复杂。

## 8. 参考资料

- **PDF**：[TF6255_TC3_Modbus_RTU_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6255_TC3_Modbus_RTU_EN.pdf) §5.1.8（接口）、§4.2（Modbus 地址映射）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6255_tc3_modbus_rtu/186540811.html
- **相关 FB / 枚举**：`ModbusRtuSlave_PcCOM` / `ModbusRtuSlave_KL6x22B` / `ModbusRtuSlave_Generic`、`ModbusRtuMasterV2_KL6x5B`（主站对端）、`MODBUS_ERRORS`、`MODBUS_UNITID`
