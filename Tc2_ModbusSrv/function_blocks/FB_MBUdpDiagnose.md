...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192782987.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBUdpDiagnose.TcPOU`](../examples/P_Demo_FB_MBUdpDiagnose.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 8（Diagnostics）的客户端功能块：对远端 Modbus 设备执行一系列诊断测试，用于检查主从站之间的通信系统、查询从站内部的各种错误状态。`nSubFnc` 选择子功能，`nReadData` 返回读回的诊断数据字。 本块是 UDP 变体：底层用无连接的 Modbus/UDP 报文传输，接口与同名 TCP 版 `FB_MBDiagnose` 完全一致，区别仅在传输层（不建立 TCP 连接，不保证可靠交付）。
...
**触发与状态机**：`bExecute` 上升沿触发一次诊断操作。FB 通过 ADS 异步把请求转交给 TwinCAT Modbus TCP 服务组件，由它向 `sIPAddr` 指定的远端 Modbus 设备发出 UDP（无连接数据报）请求。请求发出后 `bBUSY := TRUE`；收到对端响应或超时后，`bBUSY := FALSE`，并据结果置 `bError` 与 `nErrId`。`bExecute` 在 `bBUSY` 期间保持高电平不会重复触发，必须等一次执行结束、再来一个上升沿才会重发。
...
> 配套可导入文件：[`examples/P_Demo_FB_MBUdpDiagnose.TcPOU`](../examples/P_Demo_FB_MBUdpDiagnose.TcPOU)
>
...
- **替代方案对比**：替代是 ping/抓包等外部手段——无法读从站内部计数器；或裸 socket 自实现。设备级诊断本 FB 直接。 与 TCP 版 `FB_MBDiagnose` 的取舍：需要可靠交付选 TCP，需要低延迟且可容忍丢包选本 UDP 版。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.11.10
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192782987.html
- **相关**：`FB_MBDiagnose`（TCP 版）、`FB_MBReadRegs`（常规读）、`stLibVersion_Tc2_ModbusSrv`（库版本）
