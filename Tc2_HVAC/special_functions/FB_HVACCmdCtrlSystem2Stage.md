...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685102475.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACCmdCtrlSystem2Stage.TcPOU`](../examples/P_Demo_FB_HVACCmdCtrlSystem2Stage.TcPOU) |

---

## 1. 功能简述
特殊功能 FB（节能优化 / 报警 / 防冻 / 多路选择 / 能耗测量等）：`FB_HVACCmdCtrlSystem2Stage` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 9 个 VAR_INPUT 引脚与 8 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `bExternalRequest1` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bExternalRequest2` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bTimeScheduler1` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bTimeScheduler2` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `bOn1` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bOn2` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `eStateModeActuator` | `E_HVACActuatorMode` | - | 当前实际运行模式枚举回读（ModeActuator）。 |
| `bRequestScheduler1` | `BOOL` | - | 请求位（uestScheduler1），告诉上游 / 下游需要某个服务。 |
| `bRequestScheduler2` | `BOOL` | - | 请求位（uestScheduler2），告诉上游 / 下游需要某个服务。 |
| `bRequestExternal1` | `BOOL` | - | 请求位（uestExternal1），告诉上游 / 下游需要某个服务。 |
| `bRequestExternal2` | `BOOL` | - | 请求位（uestExternal2），告诉上游 / 下游需要某个服务。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `ePlantMode` | `E_HVACPlantMode` | - | 枚举 / 结构（参见 `E_HVACPlantMode`）。 |
| `eExternalRequestMode` | `E_HVACExternalRequestMode` | - | 枚举 / 结构（参见 `E_HVACExternalRequestMode`）。 |
| `tOffDelay1` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
| `tOffDelay2` | `TIME` | - | 时间参数（语义见 PDF 同名描述段）。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACCmdCtrlSystem2Stage.TcPOU`](../examples/P_Demo_FB_HVACCmdCtrlSystem2Stage.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.7.8
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685102475.html
