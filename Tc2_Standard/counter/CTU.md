...
计数值类型 `WORD`（16 位无符号），范围 0–65535。一旦计满 65535 再继续递增会回卷为 0——业务上需要更大计数范围请改用自己写 DINT/UDINT 累加。
...
- **`CU` 必须是脉冲不是电平**：常见错误是把传感器信号直接长接到 `CU`，结果只数到 1。要么用脉冲信号，要么前面串 `R_TRIG`：
  ...
  ```
- **`CV` 是 WORD 上限 65535**：超过会回卷，业务上想数 100 万件必须自己用 UDINT 累加：`IF fbRTrig.Q THEN nProductCount := nProductCount + 1; END_IF`。
