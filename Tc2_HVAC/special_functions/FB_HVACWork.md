...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685145483.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACWork.TcPOU`](../examples/P_Demo_FB_HVACWork.TcPOU) |

---

## 1. 功能简述
特殊功能 FB（节能优化 / 报警 / 防冻 / 多路选择 / 能耗测量等）：`FB_HVACWork` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 10 个 VAR_INPUT 引脚与 3 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `bResetHours` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bResetCount` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `dtSystemtime` | `DT` | - | 语义见 PDF 同名描述段。 |
| `bStartWarnLevel` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bStartAlarmLevel` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `bQAlarmLevel` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
| `bQWarnLevel` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACWork.TcPOU`](../examples/P_Demo_FB_HVACWork.TcPOU)
>
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.7.35
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685145483.html
