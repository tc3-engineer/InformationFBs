...
| Source | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_standard/delete.html |
...
**删除子串**。从 `STR` 中删除自位置 `POS` 起共 `LEN` 个字符。
...
- **位置 1 起**：`POS = 1` 表示从第一个字符开始。
- `POS + LEN - 1` 超出实际长度时行为以 PDF 实现为准（⚠️ 待人工确认）。
...
VAR
    sResult : STRING(255);   // DELETE 的返回值（监视）
    bRun    : BOOL;          // TRUE 触发一次调用
END_VAR

IF bRun THEN
    sResult := DELETE('SUXYSI',2,3);
    bRun := FALSE;
END_IF;
