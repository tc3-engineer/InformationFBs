...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187735819.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_4OCTET_FLOAT_REC.TcPOU`](../examples/P_Demo_EIB_4OCTET_FLOAT_REC.TcPOU) |

---

## 1. 功能简述

接收 EIB 网络上指定组地址的 **4 字节 IEEE 754 float telegram**，对应 KNX DPT 14.xxx 数据类型。比 `EIB_2OCTET_FLOAT_REC` 精度高 7+ 位、范围大 ±1e38，可表达工业级测量精度（流量、压力、电参数）。
...
| `rData` | `REAL` | 收到的负载即 IEC `REAL`（IEEE 754 单精度浮点，范围约 ±1e38，精度约 1e-7） |
...
**数据宽度匹配**：发送端 telegram 的负载长度必须与本 FB 类型对应（本 FB 需要 4-byte float telegram）。类型不匹配的 telegram 会被 KL6301 丢弃（`WRONG_EIB_DATA_LEN`），本 FB 永远收不到。
...
> 配套可导入文件：[`examples/P_Demo_EIB_4OCTET_FLOAT_REC.TcPOU`](../examples/P_Demo_EIB_4OCTET_FLOAT_REC.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）

> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
PROGRAM P_Demo_EIB_4OCTET_FLOAT_REC
VAR
    fbEIB_4OCTET_FLOAT_REC        : EIB_4OCTET_FLOAT_REC;
...
- **场景**：电参数、流量、压力等工业级精度模拟量
- **价值**：不损失精度的 4-byte float 接收；标准 DPT 14 实现
- **替代方案对比**：
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.4.5
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187735819.html
- **相关**：`EIB_2OCTET_FLOAT_REC`、`EIB_4OCTET_FLOAT_SEND`、`KL6301`
