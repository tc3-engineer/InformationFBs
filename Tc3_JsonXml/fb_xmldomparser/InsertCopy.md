...
| Status | `verified` |
| Example | [`examples/P_Demo_FB_XmlDomParser_InsertCopy.TcPOU`](../examples/P_Demo_FB_XmlDomParser_InsertCopy.TcPOU) |

---

## 1. 功能简述

`FB_XmlDomParser.InsertCopy()` 是 XML DOM 文档 提供的一个工具方法，在解析/构造 JSON 或 XML 文档的特定步骤中使用。具体参数语义见下文 §2 接口定义表格与 §3 行为说明；参数语义按命名约定推导自该方法在 `FB_XmlDomParser` 内的位置。
...
| `copy` | `SXmlNode` | 是否深拷贝：`TRUE` 表示拷贝节点内容并独立成新节点；`FALSE` 直接引用源节点。 |
...
> 配套可导入文件：[`examples/P_Demo_FB_XmlDomParser_InsertCopy.TcPOU`](../examples/P_Demo_FB_XmlDomParser_InsertCopy.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **PDF**：[`TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf) §4.7.57
