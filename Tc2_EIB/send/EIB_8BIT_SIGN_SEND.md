...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187769611.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_8BIT_SIGN_SEND.TcPOU`](../examples/P_Demo_EIB_8BIT_SIGN_SEND.TcPOU) |

---

## 1. 功能简述

**发送 8-bit 缩放 telegram**（DPT 5.001 % / 5.003 ° / 5.005 byte）：把 IEC `INT` 按 `Scaling_Mode` 缩放后发出。`iData` 值变化时下发；最小间隔 1 秒（PDF "data are only transferred if there is a change in the data value" + 1 sec min send time）。
...
| `iData` | `INT` | — | 数据值。**取值范围依 `Scaling_Mode`** |
| `Scaling_Mode` | `INT` | — | 0 = `iData` 是 0..100 [%]，库按 raw = iData * 255 / 100 编码；1 = 0..360 [°]，raw = iData * 255 / 360；2 = 0..255，原始 byte 直接传 |
...
**触发方式**：每周期调用本 FB；**只要数据值（iData）发生变化**，库就把新值编码成 EIB telegram 并下发到 EIB 总线。**初次调用**也会发一次（首次值通过）。
...
> 配套可导入文件：[`examples/P_Demo_EIB_8BIT_SIGN_SEND.TcPOU`](../examples/P_Demo_EIB_8BIT_SIGN_SEND.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
...
- **场景**：调光百分比、角度等 DPT 5.001/003/005 场景
- **价值**：替代手写缩放 + 编码；DPT 5 标准选择
- **替代方案对比**：
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.15
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187769611.html
- **相关**：`EIB_8BIT_SIGN_SEND_EX`（§4.2.5.16）、`EIB_8BIT_SIGN_REC`
