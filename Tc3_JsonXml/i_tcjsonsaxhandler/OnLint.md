...
| Status | `verified` |
| Example | [`examples/P_Demo_ITcJsonSaxHandler_OnLint.TcPOU`](../examples/P_Demo_ITcJsonSaxHandler_OnLint.TcPOU) |

---

## 1. 功能简述

SAX 解析回调：扫描到 LINT 值 时被调用，调用方业务代码可在此读取本次 token 内容并选择继续（返回 `S_OK`）或终止（返回 `S_FALSE`）解析。
...
> 配套可导入文件：[`examples/P_Demo_ITcJsonSaxHandler_OnLint.TcPOU`](../examples/P_Demo_ITcJsonSaxHandler_OnLint.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **PDF**：[`TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc3_JsonXml_EN.pdf) §5.1.6
