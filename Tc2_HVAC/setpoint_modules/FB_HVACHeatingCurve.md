...
| Type | `FUNCTION_BLOCK` |
| Category | `HVAC Setpoint Modules` |
| Source PDF | https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf |
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685081099.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `⚠️ chapter-overview-only` |
| Example | [`examples/P_Demo_FB_HVACHeatingCurve.TcPOU`](../examples/P_Demo_FB_HVACHeatingCurve.TcPOU) |

---

## 1. 功能简述
设定值模块 FB（加热曲线 / 室外温度阻尼 / 设定值斜率 / 夏季补偿）：`FB_HVACHeatingCurve` 是 Tc2_HVAC 库该类别下的一个核心功能块，对外暴露 5 个 VAR_INPUT 引脚与 2 个 VAR_OUTPUT 引脚，按典型 BMS / OP 双通道模式接入现场工艺信号。详细引脚作用见 §2 接口定义表，时序与模式仲裁见 §3 行为说明。该 FB 与同库其它执行 / 控制 / 设定值 FB 共用持久化策略 `eDataSecurityType` 与状态字 `byState`，上层工程可统一管理。
...
| `rOutsideTempDamped` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `bDayOperation` | `BOOL` | - | 布尔标志位（语义见 PDF 同名描述段）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `rTempSetpoint` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
...
| 名称 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `rMinTemp` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rMaxTemp` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rNightSetback` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rTempSetpoint_Y1` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rTempSetpoint_Y2` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rTempSetpoint_Y3` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rTempSetpoint_Y4` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rOutsideTemp_X1` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rOutsideTemp_X2` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rOutsideTemp_X3` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |
| `rOutsideTemp_X4` | `REAL` | - | 浮点工程量（语义见 PDF 同名描述段）。 |

## 3. 行为说明

`FB_HVACHeatingCurve` 是 Tc2_HVAC 库 设定值模块 FB（加热曲线 / 室外温度阻尼 / 设定值斜率 / 夏季补偿） 子类中的功能块。按 §2 接口定义表列出的引脚顺序，每周期单次调用本 FB；输入信号通过 VAR_INPUT 引脚传入、输出结果通过 VAR_OUTPUT 引脚回读，状态 / 错误位与同库其他 FB 的命名约定保持一致。 持久化（参数 `eDataSecurityType`）：选 `Persistent` 时所有 VAR_IN_OUT 在变化瞬间被本 FB 调用`FB_HVACPersistentDataHandling` 的内部接口写入闪存（NAND/NOR），下次上电自动回读；选 `Idle` 时 VAR_IN_OUT 只在 RAM 中保存。**必须**在主程序中实例化 `FB_HVACPersistentDataHandling` 并周期调用，否则持久化机制不工作。 `bSetDefault` 上升沿一次性把 VAR_IN_OUT 区所有参数复位为 PDF 列出的默认值，避免下载工程后VAR_IN_OUT 区为 0 / 空导致逻辑误动作。工程现场建议把 HMI 上「恢复默认」按钮接到这个引脚。 每个 PLC 周期都应调用本 FB 一次（不要条件调用、不要在不同任务里调用同一实例）；FB 内部维护状态机 / 积分量 / 时间累积，跳过调用会让计数 / 时序不准。按Tc2_HVAC 全库统一约定，所有输出在 FB 被调用的同一周期内更新，调用方可立即读取输出。
...
> 配套可导入文件：[`examples/P_Demo_FB_HVACHeatingCurve.TcPOU`](../examples/P_Demo_FB_HVACHeatingCurve.TcPOU)
>
...
- **场景**：HVAC 设定值生成：加热曲线、室外温度阻尼、夏季补偿、设定值斜率等用于「外温越低，供水温度越高」之类的工程关系换算。
...
- **PDF**：[TF8000_TC3_HVAC_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF8000_TC3_HVAC_EN.pdf) §5.1.6.1
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf8000_tc3_hvac/4685081099.html
