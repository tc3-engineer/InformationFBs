...
| Source | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_standard/wleft.html |
...
**WSTRING 版本**：取 WSTRING 左侧前 N 字符。 与 `LEFT` 同语义，但操作 `WSTRING`（双字节字符）。
...
VAR
    sResult : WSTRING(255);   // WLEFT 的返回值（监视）
    bRun    : BOOL;          // TRUE 触发一次调用
END_VAR

IF bRun THEN
    sResult := WLEFT("SUSI",3);
    bRun := FALSE;
END_IF;
