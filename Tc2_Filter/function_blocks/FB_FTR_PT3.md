...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/5855747723.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_FTR_PT3.TcPOU`](../examples/P_Demo_FB_FTR_PT3.TcPOU) |

---

## 1. 功能简述

实现一个三阶延迟环节（PT3 element），由三个串联一阶环节构成，拉普拉斯域传递函数 `G(s) = Kp / ((1 + T1·s)(1 + T2·s)(1 + T3·s))`。由配置结构 `ST_FTR_PT3` 给定增益与三个时间常数。

> 类别归属：三阶延迟环节（PT3）。属于 TF3680（TwinCAT 3 Filter）的数字滤波器函数块族，全族共用相同的『配置结构 + Configure/Call/Reset』接口范式。
...
| 名称 | 类型 | 说明（中文） |
|---|---|---|
| `stConfig` | `ST_FTR_PT3` | 滤波器配置结构。承载该滤波器全部参数（见下方『配置结构』小节） |
...
1. **配置（Configure）**：可以在声明时直接传入 `stConfig`（`fbFilter : FB_FTR_PT3(stConfig := stParams);`），也可以在运行时调用 `fbFilter.Configure(stConfig := stParams)` 初始化或重新配置。**未成功配置（`bConfigured = FALSE`）时 `Call()` 与 `Reset()` 不可用。**
...
三个实极点带来更强的高频衰减（约 -60 dB/decade）与更长的群延迟；阶跃响应仍无超调但上升更缓。三个时间常数独立可调。阶数再高、或时间常数相同的多阶场合，可改用 `FB_FTR_PTn`。
...
- **先配置后调用**：`bConfigured` 为 `TRUE` 才能 `Call()`/`Reset()`；声明时用 `fbFilter : FB_FTR_PT3(stConfig := stParams);` 可在初始化阶段直接配好。
...
> 配套可导入文件：[`examples/P_Demo_FB_FTR_PT3.TcPOU`](../examples/P_Demo_FB_FTR_PT3.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK
> 详见 [`examples/README.md`](../examples/README.md)

```iecst
// 场景：用三阶延迟环节模拟一个多级惯性被控对象（如多段加热管的热传导），或对噪声很强的信号做比 PT2 更彻底的平滑。
//   本 demo 用一个合成信号（正弦基波 + 周期性尖峰扰动）喂给 FB_FTR_PT3，
//   在线观察 fFilteredOut 如何平滑/跟随 fNoisyIn。
// 价值：提供比 PT1/PT2 更陡的滚降，三个时间常数独立可调，便于精确匹配高阶被控对象。
// 验证步骤：
...
VAR
    fbFilter      : FB_FTR_PT3;                  // 被演示的滤波器实例
    stParams      : ST_FTR_PT3 := (fKp := 1, fT1 := 0.2, fT2 := 0.1, fT3 := 0.05, fSamplingRate := 1000, nOversamples := 1, nChannels := 1);
...
- **场景**：用三阶延迟环节模拟一个多级惯性被控对象（如多段加热管的热传导），或对噪声很强的信号做比 PT2 更彻底的平滑。
- **价值**：提供比 PT1/PT2 更陡的滚降，三个时间常数独立可调，便于精确匹配高阶被控对象。
- **替代方案对比**：串三个 `FB_FTR_PT1`：要三个实例；本 FB 一次到位。三个时间常数都相同时用 `FB_FTR_PTn`（只填一个 T1 + 阶数）更省事。

## 8. 参考资料

- **PDF**：[TF3680_TC3_Filter_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF3680_TC3_Filter_EN.pdf) §5.1.7（功能块）、§5.2.1.7（配置结构）、§4.1-4.2（数字滤波器原理与参数化）、§7.1（错误码）
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf3680_tc3_filter/5855747723.html
