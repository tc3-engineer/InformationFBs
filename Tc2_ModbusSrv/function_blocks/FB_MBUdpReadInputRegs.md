...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192773771.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBUdpReadInputRegs.TcPOU`](../examples/P_Demo_FB_MBUdpReadInputRegs.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 4（Read Input Registers）的客户端功能块：从远端 Modbus 设备读取 1 到 128 个输入寄存器（16 位）。注意字节序为小端（Little Endian）。`bExecute` 上升沿触发一次读取。 本块是 UDP 变体：底层用无连接的 Modbus/UDP 报文传输，接口与同名 TCP 版 `FB_MBReadInputRegs` 完全一致，区别仅在传输层（不建立 TCP 连接，不保证可靠交付）。
...
| `nQuantity` | `WORD` | `-` | 要读取的输入寄存器（数据字）个数。值为 0 不允许 |
| `nMBAddr` | `WORD` | `-` | 要读取的输入寄存器起始地址（字偏移） |
| `cbLength` | `UDINT` | `-` | 目标缓冲区可用字节数。缓冲区至少须有 `nQuantity * 2` 字节 |
...
**角色定位**：本库的 FB_MB* 系列让 PLC 充当 Modbus **主站（master/client）**，主动去读写远端设备的输入寄存器（只读）。它与“TwinCAT 自身作为 Modbus 从站被外部 SCADA 访问”是两回事——后者由 TF6250 服务端 + 配置器 + `mb_Input_Coils` 等 GVL 数组 + ADS 映射完成，不经过本 FB。
...
- **缓冲区字节数公式**：`cbLength` 与缓冲区数组都要满足至少 `nQuantity * 2` 字节，否则读写越界（工程经验补充：用 `SIZEOF(缓冲区变量)` 赋 `cbLength` 最稳）。
- **位/字节序**：读线圈/离散输入时，结果按位打包进字节，低位在前；读寄存器时低字节在前、高字节在后（小端）。解析时注意位序与字节序。
- **UDP 不保证可靠**：丢包不会重传、无连接状态；关键写操作（如安全相关输出）应使用 TCP 版以获得可靠交付。

## 6. 最小例程

> 配套可导入文件：[`examples/P_Demo_FB_MBUdpReadInputRegs.TcPOU`](../examples/P_Demo_FB_MBUdpReadInputRegs.TcPOU)
>
...
- **场景**：读取仪表的只读输入寄存器，例如电力仪表的瞬时电流/电压/功率、传感器变送器的测量值——这些是设备侧只读的实时量。 选用 UDP 版多见于同一控制柜/同网段、对实时性敏感且能容忍偶发丢包的场景，省去 TCP 建链开销。
- **价值**：输入寄存器区只读，用本 FB（功能码 4）周期采集即可；与保持寄存器（功能码 3）区分开，避免误写。
- **替代方案对比**：替代是裸 socket 实现，或用 OPC UA（需设备支持）。对只读 Modbus 测量量，本 FB 直接通用。 与 TCP 版 `FB_MBReadInputRegs` 的取舍：需要可靠交付选 TCP，需要低延迟且可容忍丢包选本 UDP 版。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.11.4
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192773771.html
- **相关**：`FB_MBReadInputRegs`（TCP 版）、`FB_MBReadRegs` / `FB_MBReadInputRegs`（读寄存器）、`stLibVersion_Tc2_ModbusSrv`（库版本）
