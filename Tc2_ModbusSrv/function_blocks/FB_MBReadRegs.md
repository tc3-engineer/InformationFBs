...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192755467.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBReadRegs.TcPOU`](../examples/P_Demo_FB_MBReadRegs.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 3（Read Holding Registers）的客户端功能块：从远端 Modbus 设备读取 1 到 128 个输出寄存器（16 位）。每个寄存器低字节在前、高字节在后写入缓冲区。`bExecute` 上升沿触发一次读取。
...
> 配套可导入文件：[`examples/P_Demo_FB_MBReadRegs.TcPOU`](../examples/P_Demo_FB_MBReadRegs.TcPOU)
>
...
- **场景**：读取变频器、温控表、电表等 Modbus 设备的保持寄存器，例如读回设定频率、实际转速、累计电量等 16 位过程量。
- **价值**：保持寄存器是最常用的 Modbus 数据区；本 FB 把功能码 3 封装为一次调用，按字节序自动落入 `WORD` 数组，省去拼包解包。
- **替代方案对比**：替代是裸 socket 自写协议，或用厂商专有协议网关。对标准 Modbus 设备，本 FB 通用且零额外硬件。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.3
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192755467.html
- **相关**：`FB_MBUdpReadRegs`（UDP 版）、`FB_MBReadRegs` / `FB_MBReadInputRegs`（读寄存器）、`stLibVersion_Tc2_ModbusSrv`（库版本）
