...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_profinetdiag/14965552779.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_PN_IM0_READ.TcPOU`](../examples/P_Demo_FB_PN_IM0_READ.TcPOU) |

---

## 1. 功能简述

PROFINET 控制器用本功能块从指定 `PORT` 引用的设备读取全部 I&M0（Identification & Maintenance，标识与维护）数据。I&M0 帧结构对应 PROFINET 标准中的索引 `0xAFF0`，包含厂商 ID、订货号、序列号、硬件/软件版本等设备出厂标识信息，通过结构 `str_IM_0xAFF0` 一次性返回。
...
| `IM_AFF0` | `str_IM_0xAFF0` | 设备返回的 I&M0 帧，封装为结构输出（厂商 ID、订货号 `cOrderID`、序列号 `cSerialNumber`、硬件版本 `nHW_Rev`、软件版本 `strSW_Rev` 等）。 |
...
本功能块是基于 ADS 的异步功能块，内部维护「空闲 → 忙 → 完成」三态状态机。`bStart` 由 FALSE 变为 TRUE 的上升沿触发一次操作：触发后 `bBusy` 立即置 TRUE，功能块通过 ADS 把请求发往 PROFINET 控制器（由 `NETID` 与 `PORT` 寻址到目标设备）；收到设备应答后 `bBusy` 落回 FALSE，此时 `IM_AFF0` 结构 才有效，若过程出错则 `bError` 在 `bBusy` 落沿之后置 TRUE、`iErrorID` 给出 ADS 错误号。`bBusy = TRUE` 期间功能块忽略输入端的任何新触发，必须等到本次完成才能再次发起。
...
> 配套可导入文件：[`examples/P_Demo_FB_PN_IM0_READ.TcPOU`](../examples/P_Demo_FB_PN_IM0_READ.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
> 导入步骤：右键 PLC 项目下 POUs 文件夹 → Add → Existing Item → 选该文件 → OK

```iecst
PROGRAM P_Demo_FB_PN_IM0_READ
VAR
    fbFB_PN_IM0_READ : FB_PN_IM0_READ;        // FB_PN_IM0_READ 实例
...
- **场景**：设备资产盘点 / 来料校验。PLC 在设备上线时读出每个 PROFINET 从站的厂商 ID、订货号、序列号，与 MES 里登记的 BOM 比对，防止装错型号或被掉包。
- **价值**：把 PROFINET 标准的 `0xAFF0` 记录解析封装成结构，业务侧直接取 `cOrderID`/`cSerialNumber` 字段，不必手工拼 record index、解二进制帧。
- **替代方案对比**：用通用 `ADSREAD` 读 record 也能拿到原始字节，但要自己按 PROFINET 规范切字段；本 FB 免去解析。

## 8. 参考资料

- **PDF**：[`TwinCAT_3_PLC_Lib_Tc2_ProfinetDiag_EN.pdf`](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_ProfinetDiag_EN.pdf) 第 3.1.2.1 节
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_profinetdiag/14965552779.html
- **相关 FB / FC**：`FB_PN_IM1_READ`~`FB_PN_IM4_READ`（其它 I&M 记录）、`str_IM_0xAFF0`（数据结构）
