...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192764683.html |
| Verified | 2026-06-02 ✅ |
| InfoSys-checked | ✅ 2026-06-02 |
| Status | `verified` |
| Example | [`examples/P_Demo_FB_MBReadWriteRegs.TcPOU`](../examples/P_Demo_FB_MBReadWriteRegs.TcPOU) |

---

## 1. 功能简述

Modbus 功能码 23（Read/Write Multiple Registers）的客户端功能块：先读 1 到 128 个输出寄存器，再写 1 到 128 个输出寄存器。读写在同一次 Modbus 事务内完成。`bExecute` 上升沿触发一次。
...
**触发与状态机**：`bExecute` 上升沿触发一次读写操作。FB 通过 ADS 异步把请求转交给 TwinCAT Modbus TCP 服务组件，由它向 `sIPAddr` 指定的远端 Modbus 设备发出 TCP（端口 502，面向连接）请求。请求发出后 `bBUSY := TRUE`；收到对端响应或超时后，`bBUSY := FALSE`，并据结果置 `bError` 与 `nErrId`。`bExecute` 在 `bBUSY` 期间保持高电平不会重复触发，必须等一次执行结束、再来一个上升沿才会重发。
...
> 配套可导入文件：[`examples/P_Demo_FB_MBReadWriteRegs.TcPOU`](../examples/P_Demo_FB_MBReadWriteRegs.TcPOU)
>
...
- **场景**：在一次 Modbus 事务里既读回设备状态又下发新设定，例如读回当前测量值同时写入下一周期设定值，保证读写在同一往返内原子完成。
- **价值**：功能码 23 把读+写合并为一次报文往返，比分开两次（功能码 3 + 16）延迟低、且语义上更接近原子；本 FB 一次调用搞定。
- **替代方案对比**：替代是先 ReadRegs 再 WriteRegs 两次调用——两次往返、读写之间可能被打断；本 FB 合并更优。

## 8. 参考资料

- **PDF**：[TF6250_TC3_Modbus_TCP_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF6250_TC3_Modbus_TCP_EN.pdf) §6.2.9
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf6250_tc3_modbus_tcp/192764683.html
- **相关**：`FB_MBUdpReadWriteRegs`（UDP 版）、`FB_MBReadRegs` + `FB_MBWriteRegs`（分开读/写）、`stLibVersion_Tc2_ModbusSrv`（库版本）
