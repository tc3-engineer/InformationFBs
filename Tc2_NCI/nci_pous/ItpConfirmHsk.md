...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tf5100_tc3_nc_i/3284130955.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_ItpConfirmHsk.TcPOU`](../examples/P_Demo_ItpConfirmHsk.TcPOU) |

---

## 1. 功能简述

`ItpConfirmHsk` 用于在 NC 程序执行到一个『带握手的 M 函数』（Handshake M-Function，如 M50 触发外部动作）时，由 PLC 把动作做完之后『回执』给 NCI，让解释器继续往下走。若通道 Override 为 0 或处在 EStop 中，该 FB 的 `bBusy` 不会消失，必须持续调用直到状态恢复。
...
`ItpConfirmHsk` 走 ADS 同步调用流程：在 `bExecute` 检测到上升沿时把命令打包发给 NCI、进入 `bBusy = TRUE` 状态等回包；NC 端接受后 `bBusy = FALSE`，若执行失败则 `bErr = TRUE` 并把错误号写入 `nErrId`。注意 PDF 原文明确指出 `bBusy` 监视的是 NC 端『接受』命令的时间，不是命令真正执行完成的时间——也就是说 `bBusy = FALSE` 不等于动作做完，只是 ADS 调用收到了应答。
...
**典型调用方式**：例程见 §6；本 FB 的 VAR_IN_OUT 引脚为 `sNciToPlc : NCTOPLC_NCICHANNEL_REF`、`sPlcToNci : PLCTONC_NCICHANNEL_REF`。VAR_IN_OUT 表示传引用，调用方在 PLC 端必须有一个对应类型的本地实例；对循环通道接口（`NCTOPLC_NCICHANNEL_REF` / `PLCTONC_NCICHANNEL_REF`）类型要在 System Manager 里Link 给 NCI 通道的对应接口（`AT %I*` / `AT %Q*`），不 Link 时 FB 拿到的镜像全 0。
...
> 配套可导入文件：[`examples/P_Demo_ItpConfirmHsk.TcPOU`](../examples/P_Demo_ItpConfirmHsk.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
>
...
- **PDF**：[TF5100_TC3_NC_I_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TF5100_TC3_NC_I_EN.pdf) §7.1.2.1
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tf5100_tc3_nc_i/3284130955.html
- **相关 FB / FC**：`ItpIsHskMFunc`（查询有无 M 函数等待）、`ItpGetHskMFunc`（取 M 函数号）
