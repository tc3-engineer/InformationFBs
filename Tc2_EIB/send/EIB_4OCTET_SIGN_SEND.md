...
| Source InfoSys | https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187765003.html |
| Verified | 2026-06-03 ✅ |
| InfoSys-checked | ✅ 2026-06-03 |
| Status | `verified` |
| Example | [`examples/P_Demo_EIB_4OCTET_SIGN_SEND.TcPOU`](../examples/P_Demo_EIB_4OCTET_SIGN_SEND.TcPOU) |

---

## 1. 功能简述

**发送 DPT 13.xxx (4-byte signed int) telegram**：把 IEC `DINT` 值经库编码成 EIB 标准格式，发送到指定 `Group_Address`。**只在 `iData` 值变化时下发**——周期重发要用 `EIB_4OCTET_SIGN_SEND_EX`。
...
| `iData` | `DINT` | — | 数据值（IEC `DINT`），库编码为 EIB 4 byte signed（DPT 13.xxx）。**PDF VAR_INPUT 引脚为 `iData`**（PDF 描述表把它列成 `uiData` 是表头排版错；以代码块为准） |
...
> 配套可导入文件：[`examples/P_Demo_EIB_4OCTET_SIGN_SEND.TcPOU`](../examples/P_Demo_EIB_4OCTET_SIGN_SEND.TcPOU)（TwinCAT 3 原生 .TcPOU，可直接拖入 XAE 的 PLC POUs 文件夹）
...
- **PDF**：[TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf](https://download.beckhoff.com/download/document/automation/twincat3/TwinCAT_3_PLC_Lib_Tc2_EIB_EN.pdf) §4.2.5.11
- **InfoSys topic**：https://infosys.beckhoff.com/content/1033/tcplclib_tc2_eib/187765003.html
- **相关**：`EIB_4OCTET_SIGN_SEND_EX`（同库 §4.2.5.12）、`EIB_4OCTET_SIGN_REC`
